...
describe('parseEmlFile', () => {
  ...
  it('falls back to "(Sem assunto)" when subject header is missing', async () => {
    const eml = [
  ...
  it('returns "Remetente desconhecido" when From header is missing', async () => {
    const eml = [
      'To: bob@example.com',
      'Subject: Anonymous',
      'Content-Type: text/plain',
    ...
    const result = await parseEmlFile(makeEmlFile(eml));
    expect(result.from).toBe('Remetente desconhecido');
  });
});
...
describe('parseMsgFile', () => {
  ...
  it('returns "Remetente desconhecido" when sender fields are missing', async () => {
    setMockMsgData({
      subject: 'Anonymous',
    });
    const result = await parseMsgFile(makeMsgFile());
  ...
  it('falls back to "(Sem assunto)" when subject is missing', async () => {
    setMockMsgData({
      senderName: 'Alice',
      senderEmail: 'alice@example.com',
    });
