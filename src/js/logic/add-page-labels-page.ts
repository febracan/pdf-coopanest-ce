...
const STYLE_LABEL_FALLBACKS: Record<PageLabelStyleName, string> = {
  DecimalArabic: 'Arábico decimal',
  LowercaseRoman: 'Romano minúsculo',
  UppercaseRoman: 'Romano maiúsculo',
  LowercaseLetters: 'Letras minúsculas',
  UppercaseLetters: 'Letras maiúsculas',
  NoLabelPrefixOnly: 'Somente prefixo, sem rótulo',
};
...
async function handleFiles(files: FileList) {
  ...
  if (
    ...
    showAlert(
      translate('tools:addPageLabels.invalidFileTitle', 'Arquivo inválido'),
      translate(
  ...
  showLoader(translate('tools:addPageLabels.loadingPdf', 'Carregando PDF...'));
  try {
    ...
    if (pdfDoc.isEncrypted) {
      showAlert(
        translate('tools:addPageLabels.protectedPdfTitle', 'PDF protegido'),
        translate(
          'tools:addPageLabels.protectedPdfMessage',
          'Este PDF é protegido por senha. Use primeiro a ferramenta Descriptografar ou Alterar permissões.'
        )
      );
      resetState();
      return;
    ...
    showAlert(
      ...
      translate(
        ...
        'Falha ao carregar o arquivo PDF. O arquivo pode estar inválido, corrompido ou protegido por senha.'
      )
    );
  } finally {
    hideLoader();
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = translate(
    ...
    `${formatBytes(pageState.file.size)} • ${pageState.pageCount} páginas`,
    {
...
function renderRules() {
  ...
  pageState.rules.forEach((rule, index) => {
    ...
    rangeInput.placeholder = translate(
      ...
      'Todas as páginas, ou ex.: 1-4, 7, ímpares'
    );
    ...
    progressText.textContent = translate(
      ...
      'Continuar a numeração entre intervalos separados'
    );
    ...
    note.textContent = translate(
      ...
      'Exemplos: 1-4 para páginas iniciais em algarismos romanos, 15-20 com prefixo A- e valor inicial 0, ou ímpares com continuação ativada.'
    );
...
async function addPageLabels() {
  ...
  showLoader(
    translate(
      'tools:addPageLabels.applyingLabels',
      'Aplicando rótulos de página...'
    )
  );
  ...
  try {
      ...
      try {
        ...
        throw new Error(
          translate(
            ...
            `A regra ${index + 1} tem um intervalo de páginas inválido: ${trimmedRange || 'todas as páginas'}`,
            {
              number: index + 1,
              range:
                trimmedRange ||
                translate('tools:addPageLabels.allPages', 'todas as páginas'),
            }
          ),
          { cause: error }
        );
    ...
    if (!outputBytes || outputBytes.length === 0) {
      throw new Error(
        translate(
          ...
          'O CoherentPDF gerou um arquivo vazio.'
        )
      );
    }
    ...
    showAlert(
      ...
      translate(
        ...
        'Rótulos de página adicionados com sucesso!'
      ),
      'success',
      () => {
        resetState();
    ...
    const message =
      error instanceof Error
        ...
        : translate(
            ...
            'Não foi possível adicionar os rótulos de página.'
          );
    showAlert(translate('common.error', 'Erro'), message);
  } finally {
    if (cpdf && pdf) {
      try {
        cpdf.deletePdf(pdf);
