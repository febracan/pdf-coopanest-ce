    ...
    .map((angle, idx) => {
      ...
      return `
        <div class="flex items-center gap-2 text-sm py-1">
          ...
          <span class="text-gray-300">Página ${idx + 1}:</span>
          <span class="${color}">${angle.toFixed(2)}°</span>
          ${wasCorrected ? '<span class="text-green-400 text-xs">(corrigida)</span>' : ''}
        </div>
      `;
    })
    .join('');
...
async function processDeskew(): Promise<void> {
  ...
  showLoader('Inicializando PyMuPDF...');
  ...
  try {
    ...
    for (const file of selectedFiles) {
      showLoader(`Corrigindo inclinação de ${file.name}...`);
    ...
    showAlert(
      'Sucesso',
      `Inclinação corrigida em ${selectedFiles.length} arquivo(s). ${selectedFiles.length > 1 ? 'Os downloads de todos os arquivos foram iniciados.' : ''}`
    );
  } catch (error) {
    hideLoader();
    ...
    showAlert(
      'Erro',
      `Falha ao corrigir a inclinação do PDF: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
    );
  }
}
