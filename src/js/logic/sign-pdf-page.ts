...
async function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(result.file.size)} • ${result.pdf.numPages} páginas`;
  result.pdf.destroy();
}
...
async function setupSignTool() {
  ...
  showLoader('Carregando visualizador de PDF...');
...
async function applyAndSaveSignatures() {
  if (!signState.viewerReady || !signState.viewerIframe) {
    showAlert(
      ...
      'Aguarde o carregamento do visualizador de PDF.'
    );
    return;
  }
  ...
  try {
    ...
    if (!viewerWindow || !viewerWindow.PDFViewerApplication) {
      showAlert(
        'Visualizador não está pronto',
        'O visualizador de PDF ainda está inicializando.'
      );
      return;
    }
    ...
    if (shouldFlatten) {
      showLoader('Achatando e salvando o PDF...');
      ...
      showAlert(
        'Sucesso',
        'PDF assinado baixado com sucesso!',
        'success',
        () => {
          resetState();
        }
    ...
    showAlert(
      'Falha na exportação',
      'Não foi possível exportar o PDF assinado. Tente novamente.'
    );
  }
}
