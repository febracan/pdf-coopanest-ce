...
worker.onmessage = function (e) {
  ...
  if (e.data.status === 'success') {
    ...
    if (attachments.length === 0) {
      showAlert(
        'Nenhum anexo',
        'O(s) arquivo(s) PDF não contêm anexos para extrair.'
      );
      resetState();
      return;
    }
    ...
    zip.generateAsync({ type: 'blob' }).then(function (zipBlob) {
      ...
      showAlert(
        'Sucesso',
        `${attachments.length} anexo(s) extraído(s) com sucesso!`
      );

      showStatus(
        `Extração concluída! ${attachments.length} anexo(s) no arquivo ZIP (${formatBytes(totalSize)}). Download iniciado.`,
        'success'
      );
...
worker.onerror = function (error) {
  ...
  showStatus(
    'Ocorreu um erro no worker. Verifique o console para mais detalhes.',
    'error'
  );
...
async function updateUI() {
  ...
  if (pageState.files.length > 0) {
    ...
    countSpan.textContent = `${pageState.files.length} arquivo(s) PDF selecionado(s)`;
...
async function extractAttachments() {
  if (pageState.files.length === 0) {
    showStatus('Nenhum arquivo', 'error');
    return;
  }
  ...
  try {
    ...
    showStatus(
      `Extraindo anexos de ${pageState.files.length} arquivo(s)...`,
      'info'
    );
    ...
    showStatus(
      `Erro ao ler os arquivos: ${error instanceof Error ? error.message : 'Ocorreu um erro desconhecido'}`,
      'error'
    );
