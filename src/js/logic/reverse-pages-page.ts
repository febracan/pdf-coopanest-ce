...
async function reversePages() {
  if (reverseState.files.length === 0) {
    showAlert('Nenhum arquivo', 'Selecione um ou mais arquivos PDF.');
    return;
  }

  try {
    const decryptedFiles = await batchDecryptIfNeeded(reverseState.files);
    showLoader('Invertendo ordem das páginas...');
    ...
    showAlert(
      'Sucesso',
      'As páginas foram invertidas com sucesso!',
      'success',
      function () {
        resetState();
      }
    ...
    showAlert(
      'Erro',
      'Não foi possível inverter as páginas do PDF. Verifique se seus arquivos são PDFs válidos.'
    );
  } finally {
    hideLoader();
  }
