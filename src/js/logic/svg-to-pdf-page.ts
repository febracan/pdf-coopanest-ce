...
function handleFiles(newFiles: FileList) {
  ...
  if (validFiles.length < newFiles.length) {
    showAlert(
      'Arquivos inválidos',
      'Alguns arquivos foram ignorados. Apenas gráficos SVG são permitidos.'
    );
  }
...
async function convertToPdf() {
  if (files.length === 0) {
    showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo SVG.');
    return;
  }

  showLoader('Criando PDF a partir dos arquivos SVG...');
