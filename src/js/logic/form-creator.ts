...
if (gotoPageBtn && gotoPageInput) {
  gotoPageBtn.addEventListener('click', () => {
    ...
    if (!isNaN(pageNum) && pageNum >= 1 && pageNum <= pages.length) {
      ...
      alert(`Digite um número de página válido entre 1 e ${pages.length}`);
    }
  });
...
function renderField(field: FormField): void {
  ...
  if (field.type === 'text') {
    ...
    contentEl.textContent = field.label || 'Botão';
    ...
    contentEl.innerHTML =
      '<div class="flex flex-col items-center"><i data-lucide="pen-tool" class="w-6 h-6 mb-1"></i><span class="text-[10px]">Assine aqui</span></div>';
    ...
    contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1"><i data-lucide="image" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">${escapeHtml(field.label || 'Clique para enviar imagem')}</span></div>`;
    setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
    ...
    if (field.barcodeValue) {
      try {
        ...
        contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1 text-gray-400"><i data-lucide="qr-code" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">Dados inválidos</span></div>`;
        setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
      }
    } else {
      contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1 text-gray-400"><i data-lucide="qr-code" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">Código de barras</span></div>`;
      setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
    }
  }
...
function showProperties(field: FormField): void {
  ...
  if (field.type === 'text') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Valor</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Comprimento máximo (0 para ilimitado)</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Dividir em caixas (0 para desativar)</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Tamanho da fonte</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Cor do texto</label>
            <input type="color" id="propTextColor" value="${field.textColor}">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Alinhamento</label>
            ...
            <option value="left" ${field.alignment === 'left' ? 'selected' : ''}>Esquerda</option>
            <option value="center" ${field.alignment === 'center' ? 'selected' : ''}>Centro</option>
            <option value="right" ${field.alignment === 'right' ? 'selected' : ''}>Direita</option>
            </select>
        </div>
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded mt-2">
            <label for="propMultiline" class="text-xs font-semibold text-gray-300">Multilinha</label>
    ...
    specificProps = `
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded">
            <label for="propChecked" class="text-xs font-semibold text-gray-300">Estado marcado</label>
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Nome do grupo (deve ser igual para o grupo)</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Valor de exportação</label>
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Opções (uma por linha ou separadas por vírgula)</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Opção selecionada</label>
        ...
        <div class="text-xs text-gray-400 italic mt-2">
            Para preencher ou alterar as opções, use nossa ferramenta de Preenchimento de Formulário PDF.
        </div>
        `;
  } else if (field.type === 'button') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Rótulo</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Ação</label>
            <select id="propAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                <option value="none" ${field.action === 'none' ? 'selected' : ''}>Nenhuma</option>
                <option value="reset" ${field.action === 'reset' ? 'selected' : ''}>Redefinir formulário</option>
                <option value="print" ${field.action === 'print' ? 'selected' : ''}>Imprimir formulário</option>
                <option value="url" ${field.action === 'url' ? 'selected' : ''}>Abrir URL</option>
                <option value="js" ${field.action === 'js' ? 'selected' : ''}>Executar Javascript</option>
                <option value="showHide" ${field.action === 'showHide' ? 'selected' : ''}>Mostrar/Ocultar campo</option>
            </select>
        </div>
        ...
        <div id="propJsContainer" class="${field.action === 'js' ? '' : 'hidden'}">
            <label class="block text-xs font-semibold text-gray-300 mb-1">Código Javascript</label>
        ...
        <div id="propShowHideContainer" class="${field.action === 'showHide' ? '' : 'hidden'}">
            <div class="mb-2">
                <label class="block text-xs font-semibold text-gray-300 mb-1">Campo de destino</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-300 mb-1">Visibilidade</label>
                <select id="propVisibilityAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="show" ${field.visibilityAction === 'show' ? 'selected' : ''}>Mostrar</option>
                    <option value="hide" ${field.visibilityAction === 'hide' ? 'selected' : ''}>Ocultar</option>
                    <option value="toggle" ${field.visibilityAction === 'toggle' ? 'selected' : ''}>Alternar</option>
                </select>
            </div>
        </div>
        `;
    ...
    specificProps = `
        <div class="text-xs text-gray-400 italic mb-2">
            Campos de assinatura são campos de assinatura AcroForm e só ficam visíveis em um visualizador de PDF avançado.
        </div>
        `;
  } else if (field.type === 'date') {
    const formats = [
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Formato da data</label>
        ...
        <div id="customFormatContainer" class="${isCustom ? '' : 'hidden'} mt-2">
            <label class="block text-xs font-semibold text-gray-300 mb-1">Formato personalizado</label>
            <input type="text" id="propCustomFormat" value="${isCustom ? escapeHtml(field.dateFormat ?? '') : ''}" placeholder="ex.: dd/mm/yyyy HH:MM:ss" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
        </div>
        <div class="mt-3 p-2 bg-gray-700 rounded">
            <span class="text-xs text-gray-400">Exemplo do formato atual:</span>
        ...
        <div class="bg-blue-900/30 border border-blue-700/50 rounded p-2 mt-2">
            <p class="text-xs text-blue-200">
                ...
                <span><strong>Observação do navegador:</strong> Firefox e Chrome podem exibir o formato nativo do seletor de data durante a seleção. O formato correto será aplicado quando você terminar de inserir a data.</span>
            </p>
        </div>
        `;
  } else if (field.type === 'image') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Rótulo / Instrução</label>
            <input type="text" id="propLabel" value="${escapeHtml(field.label)}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
        </div>
        <div class="text-xs text-gray-400 italic mt-2">
            Clicar neste campo no PDF abrirá um seletor de arquivos para enviar uma imagem.
        </div>
        `;
  } else if (field.type === 'barcode') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Formato do código de barras</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Valor do código de barras</label>
  ...
  const propertiesHtml = `
    <div class="space-y-3">
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">Nome do campo ${field.type === 'radio' ? '(nome do grupo)' : ''}</label>
      ...
      ${
        ...
        (existingRadioGroups.size > 0 ||
          ...
          ? `
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">Grupos de opção existentes</label>
        <select id="existingGroups" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
          <option value="">-- Selecionar grupo existente --</option>
        ...
        <p class="text-xs text-gray-400 mt-1">Selecione para adicionar este botão a um grupo existente</p>
      </div>
      `
          : ''
      }
      ${specificProps}
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">Dica / Texto de ajuda</label>
        <input type="text" id="propTooltip" value="${escapeHtml(field.tooltip)}" placeholder="Descrição para leitores de tela" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propRequired" ${field.required ? 'checked' : ''} class="mr-2">
        <label for="propRequired" class="text-xs font-semibold text-gray-300">Obrigatório</label>
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propReadOnly" ${field.readOnly ? 'checked' : ''} class="mr-2">
        <label for="propReadOnly" class="text-xs font-semibold text-gray-300">Somente leitura</label>
      </div>
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">Cor da borda</label>
      ...
      <div class="flex items-center">
        ...
        <label for="propHideBorder" class="text-xs font-semibold text-gray-300">Ocultar borda</label>
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propTransparentBackground" ${field.transparentBackground ? 'checked' : ''} class="mr-2">
        <label for="propTransparentBackground" class="text-xs font-semibold text-gray-300">Fundo transparente</label>
      </div>
  ...
  const validateName = (newName: string): boolean => {
    if (!newName) {
      nameError.textContent = 'O nome do campo não pode ficar vazio';
    ...
    if (isDuplicateInFields || isDuplicateInPdf) {
      nameError.textContent = `O nome de campo "${newName}" já existe neste ${isDuplicateInPdf ? 'PDF' : 'formulário'}. Tente usar um nome único.`;
      nameError.classList.remove('hidden');
      propName.classList.add('border-red-500');
      return false;
    }
  ...
  if (field.type === 'text') {
    ...
    const barcodeFormatHints: Record<string, string> = {
      qrcode: 'Qualquer texto, URL ou dado',
      code128: 'Caracteres ASCII (letras, números, símbolos)',
      code39: 'Letras maiúsculas A-Z, dígitos 0-9 e - . $ / + % ESPAÇO',
      ean13: 'Exatamente 12 ou 13 dígitos',
      upca: 'Exatamente 11 ou 12 dígitos',
      datamatrix: 'Qualquer texto, URL ou dado',
      pdf417: 'Qualquer texto, URL ou dado',
    };
...
function hideProperties(): void {
  propertiesPanel.innerHTML =
    '<p class="text-gray-500 text-sm">Selecione um campo para editar as propriedades</p>';
}
...
downloadBtn.addEventListener('click', async () => {
  ...
  if (conflictsWithPdf.length > 0) {
    const conflictList = [...new Set(conflictsWithPdf)]
      ...
      .join(', ');
    showModal(
      'Conflito de nome de campo',
      `Os seguintes nomes de campo já existem no PDF enviado: ${conflictList}. Renomeie esses campos antes de baixar.`,
      'error'
    );
    return;
  }
  ...
  if (duplicates.length > 0) {
    ...
    showModal(
      ...
      `Os seguintes nomes de campo são usados mais de uma vez: ${duplicateList}. Renomeie esses campos para usar nomes únicos antes de baixar.`,
      'error'
    );
    return;
  }
  ...
  if (fields.length === 0) {
    alert('Adicione pelo menos um campo antes de baixar.');
    return;
  }

  if (pages.length === 0) {
    alert('Nenhuma página encontrada. Crie um PDF em branco ou envie um.');
    return;
  }
  ...
  try {
    ...
    pdfDoc.setTitle('Formulário preenchível');
    pdfDoc.setAuthor('Coopanest PDF');
    ...
    if (
      ...
      if (existingRadioGroups.has(fieldName)) {
        ...
        showModal(
          'Nome de campo duplicado',
          `Já existe um campo chamado "${fieldName}". Renomeie este campo para usar um nome único antes de baixar.`,
          'error'
        );
      }
    } else {
      showModal('Erro', 'Erro ao gerar o PDF: ' + errorMessage, 'error');
    }
  }
});
...
async function handlePdfUpload(file: File) {
  try {
    ...
    showModal(
      'Erro',
      'Erro ao carregar o arquivo PDF. Tente novamente com um PDF válido.',
      'error'
    );
  }
}
...
resetBtn.addEventListener('click', () => {
  if (fields.length > 0 || pages.length > 0) {
    if (
      confirm(
        'Tem certeza de que deseja redefinir? Todo o seu trabalho será perdido.'
      )
    ) {
      resetToInitial();
    }
