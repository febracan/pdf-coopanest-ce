...
async function convert() {
  ...
  if (currentMode === 'upload' && files.length === 0) {
    showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo de texto.');
    return;
  }
  ...
  if (currentMode === 'text') {
    ...
    if (!textInput.value.trim()) {
      showAlert('Nenhum texto', 'Digite algum texto para converter.');
      return;
    }
  }
  ...
  try {
    ...
    showAlert(
      'Sucesso',
      'Texto convertido para PDF com sucesso!',
      'success',
      () => {
        resetState();
      }
    ...
    showAlert(
      'Erro',
      `Falha ao converter o texto para PDF. ${e instanceof Error ? e.message : ''}`
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        'Arquivos inválidos',
        'Alguns arquivos foram ignorados. Apenas arquivos de texto são permitidos.'
      );
    }
