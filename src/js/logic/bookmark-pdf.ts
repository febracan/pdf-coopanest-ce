        ...
        <div class="flex items-center gap-2">
          <label class="flex items-center gap-1 text-xs">
            <input type="checkbox" id="modal-use-destination" class="w-4 h-4" ${hasDestination ? 'checked' : ''}>
              <span class="text-gray-700">Definir destino personalizado</span>
                </label>
                </div>
  ...
  <label class="text-xs text-gray-600">Zoom(%)</label>
    <select id="modal-dest-zoom" class="w-full px-2 py-1 border border-gray-300 rounded text-sm text-gray-900">
      <option value="">Herdar</option>
        <option value="0">Ajustar à página</option>
                ...
                <option value="125">125%</option>
                  <option value="150">150%</option>
                    <option value="200">200%</option>
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <div>
                        <label class="text-xs text-gray-600">Posição X</label>
...
class="w-full px-2 py-1 border border-gray-300 rounded text-sm text-gray-900" />
  ...
  <div>
  <label class="text-xs text-gray-600">Posição Y</label>
...
class="w-full px-2 py-1 border border-gray-300 rounded text-sm text-gray-900" />
  ...
  <button id="modal-pick-destination" class="w-full px-3 py-2 btn-gradient text-white rounded text-xs !flex items-center justify-center gap-1">
    <i data-lucide="crosshair" class="w-3 h-3"></i> Clique no PDF para escolher o local
      </button>
      <p class="text-xs text-gray-500 italic">Clique no botão acima e depois clique no PDF no ponto para onde você quer que o marcador vá</p>
        </div>
        </div>
        </div>
          `;
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-2">${escapeHTML(field.label)}</label>
            <div id="modal-preview" class="style-preview bg-gray-50">
              <span id="preview-text" style="font-size: 16px;">Texto de prévia</span>
                </div>
                </div>
                  `;
        }
  ...
  <div class="flex gap-2 justify-end">
    <button id="modal-cancel" class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-700">Cancelar</button>
      <button id="modal-confirm" class="px-4 py-2 rounded btn-gradient text-white">Confirmar</button>
        </div>
        </div>
          `;
    ...
    function updatePreview(): void {
      ...
      if (previewText) {
        ...
        const titleVal = titleInputEl ? titleInputEl.value : 'Texto de prévia';
        const color = colorSelectEl ? colorSelectEl.value : '';
        const style = styleSelectEl ? styleSelectEl.value : '';

        previewText.textContent = titleVal || 'Texto de prévia';
...
function showConfirmModal(message: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    modal.innerHTML = `
  <div class="p-6">
    <h3 class="text-xl font-bold text-gray-800 mb-4">Confirmar ação</h3>
...
resetBtn?.addEventListener('click', async () => {
  const confirmed = await showConfirmModal(
    'Redefinir e voltar ao envio de arquivos? Todas as alterações não salvas serão perdidas.'
  );
  if (confirmed) {
    resetToUploader();
  }
...
deleteAllBtn?.addEventListener('click', async () => {
  if (bookmarkTree.length === 0) {
    await showAlertModal('Informação', 'Nenhum marcador para excluir.');
    return;
  }
...
batchDeleteBtn?.addEventListener('click', async () => {
  ...
  const confirmed = await showConfirmModal(
    `Excluir ${selectedBookmarks.size} marcador(es)? `
  );
  if (!confirmed) return;
...
csvInput?.addEventListener('change', async (e: Event) => {
  ...
  await showAlertModal(
    'CSV carregado',
    `${csvBookmarks.length} marcadores carregados do CSV. Agora envie seu PDF.`
  );
});
...
jsonInput?.addEventListener('change', async (e: Event) => {
  ...
  try {
    ...
    await showAlertModal(
      'JSON carregado',
      'Marcadores carregados do JSON. Agora envie seu PDF.'
    );
  } catch {
    await showAlertModal('Erro', 'Formato de JSON inválido');
  }
});
...
async function renderPage(
  ...
  pageIndicator!.textContent = `Página ${num} / ${pdfJsDoc.numPages}`;
...
function createNodeElement(node: BookmarkNode, level = 0): HTMLLIElement {
  ...
  titleDiv.innerHTML = `
                ...
                <span class="text-xs text-gray-500">Página ${escapeHTML(String(node.page))}</span>
            `;
      ...
      [
        ...
        },
        {
          ...
          options: [
            ...
            { value: 'bold-italic', label: 'Negrito e itálico' },
          ],
        },
        {
          type: 'destination',
  ...
  deleteBtn.addEventListener('click', async (e: MouseEvent) => {
    e.stopPropagation();
    const confirmed = await showConfirmModal(`Excluir "${node.title}"?`);
    if (confirmed) {
...
addTopLevelBtn?.addEventListener('click', async () => {
  ...
  if (!title) {
    await showAlertModal('Erro', 'Digite um título.');
    return;
  }
...
csvImportHidden?.addEventListener('change', async (e: Event) => {
  ...
  if (imported.length > 0) {
    ...
    await showAlertModal(
      'Sucesso',
      `${imported.length} marcadores importados!`
    );
  }
...
exportCsvBtn?.addEventListener('click', () => {
  ...
  if (bookmarkTree.length === 0) {
    showAlertModal('Erro', 'Nenhum marcador para exportar!');
    return;
  }
...
extractExistingBtn?.addEventListener('click', async () => {
  ...
  if (extracted.length > 0) {
    const confirmed = await showConfirmModal(
      `${extracted.length} marcadores existentes encontrados. Substituir os marcadores atuais?`
    );
    if (confirmed) {
      bookmarkTree = extracted;
      saveState();
    ...
    await showAlertModal(
      'Informação',
      'Nenhum marcador existente encontrado neste PDF.'
    );
  }
});
...
downloadBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    await showAlertModal(
      'Erro',
      'Erro ao salvar o PDF. Verifique o console para detalhes.'
    );
  }
});
