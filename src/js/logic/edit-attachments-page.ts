...
worker.onmessage = function (e) {
  ...
  if (data.status === 'success' && data.attachments !== undefined) {
    ...
    showAlert(
      'Sucesso',
      'Anexos atualizados com sucesso!',
      'success',
      function () {
        resetState();
      }
...
function displayAttachments(attachments: AttachmentInfo[]) {
  ...
  removeAllBtn.onclick = function () {
    ...
    if (allSelected) {
      ...
      removeAllBtn.textContent = 'Desmarcar todos';
    }
  };
  ...
  for (const attachment of attachments) {
    ...
    if (attachment.page === 0) {
      levelSpan.textContent = 'Anexo em nível de documento';
    } else {
...
async function loadAttachments() {
  ...
  try {
    ...
    hideLoader();
    showAlert('Erro', 'Falha ao carregar os anexos do PDF.');
  }
}

async function saveChanges() {
  if (!pageState.file) {
    showAlert('Erro', 'Nenhum arquivo PDF carregado.');
    return;
  }

  if (pageState.attachmentsToRemove.size === 0) {
    showAlert('Nenhuma alteração', 'Nenhum anexo selecionado para remoção.');
    return;
  }
  ...
  try {
    ...
    hideLoader();
    showAlert('Erro', 'Falha ao editar os anexos.');
  }
}
