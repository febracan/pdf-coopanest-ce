...
export async function processAndSave() {
  showLoader('Construindo novo PDF...');
  try {
    const grid = document.getElementById('page-grid');
    ...
    if (invalidIndices.length > 0) {
      ...
      showAlert(
        'Erro',
        'Algumas páginas não puderam ser processadas. Tente novamente.'
      );
      return;
    }
    ...
    showAlert(
      'Erro',
      'Falha ao salvar o novo PDF. Verifique o console para mais detalhes.'
    );
  } finally {
    hideLoader();
  }
