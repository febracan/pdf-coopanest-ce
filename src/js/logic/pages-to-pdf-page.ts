...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const convert = async () => {
    if (state.files.length === 0) {
      showAlert(
        'Nenhum arquivo',
        `Selecione pelo menos um arquivo ${FILETYPE_NAME}.`
      );
      return;
    }
    ...
    try {
      ...
      if (state.files.length === 1) {
        ...
        showAlert(
          'Conversão concluída',
          `${file.name} convertido para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      } else {
        ...
        for (let i = 0; i < state.files.length; i++) {
          ...
          const zipEntryName = deduplicateFileName(
            withPdfSuffix(baseName, 'convertido'),
            usedNames
          );
        ...
        downloadFile(
          zipBlob,
          withZipSuffix(state.files[0]?.name || 'pages-to-pdf', 'convertido')
        );

        hideLoader();
        showAlert(
          'Conversão concluída',
          `${state.files.length} arquivos convertidos para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      }
