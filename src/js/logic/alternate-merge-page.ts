...
async function updateUI() {
  ...
  if (pageState.files.length > 0) {
    ...
    infoSpan.textContent = `${pageState.files.length} arquivos PDF selecionados`;
    ...
    showLoader('Carregando arquivos PDF...');
    fileList.innerHTML = '';
...
async function mixPages() {
  if (pageState.pdfBytes.size < 2) {
    showAlert(
      ...
      'Envie ao menos dois arquivos PDF para alternar e mesclar.'
    );
    return;
  }
  ...
  try {
    ...
    if (filesToMerge.length < 2) {
      showAlert('Erro', 'São necessários pelo menos dois PDFs válidos.');
      hideLoader();
      return;
    }
    ...
    const message =
      e instanceof Error ? e.message : 'Ocorreu um erro ao mesclar os PDFs.';
