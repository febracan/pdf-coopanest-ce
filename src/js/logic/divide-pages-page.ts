...
async function updateUI() {
  ...
  if (pageState.file) {
    ...
    try {
      ...
      metaSpan.textContent = `${formatBytes(pageState.file.size)} • ${pageState.totalPages} páginas`;
...
async function dividePages() {
  ...
  if (pageRangeValue === '' || pageRangeValue === 'all') {
    ...
    if (pagesToDivide.size === 0) {
      showAlert(
        'Intervalo inválido',
        'Digite um intervalo de páginas válido (ex.: 1-5, 8, 11-13).'
      );
      return;
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      showLoader(`Processando página ${pageNum} de ${pages.length}...`);
    ...
    showAlert(
      'Sucesso',
      'As páginas foram divididas com sucesso!',
      'success',
      function () {
        resetState();
      }
