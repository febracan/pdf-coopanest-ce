...
async function runSanitize() {
  if (!pageState.file) {
    showAlert('Erro', 'Nenhum documento PDF carregado.');
    return;
  }
  ...
  if (loaderText) loaderText.textContent = 'Higienizando PDF...';
  ...
  try {
    ...
    showAlert('Erro', `Ocorreu um erro durante a higienização: ${msg}`);
  } finally {
    if (loaderModal) loaderModal.classList.add('hidden');
  }
}
