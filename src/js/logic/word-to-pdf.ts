...
export async function wordToPdf() {
  ...
  if (!file) {
    showAlert('Nenhum arquivo', 'Envie primeiro um arquivo .docx.');
    return;
  }

  showLoader('Preparando a prévia...');
  ...
  try {
    ...
    showAlert(
      'Erro na prévia',
      `Não foi possível gerar uma prévia. O arquivo pode estar corrompido ou conter recursos não suportados. Erro: ${e.message}`
    );
  }
}
