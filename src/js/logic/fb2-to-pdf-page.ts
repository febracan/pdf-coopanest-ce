  ...
  const convertToPdf = async () => {
    try {
      ...
      if (state.files.length === 1) {
        ...
        showAlert(
          'Conversão concluída',
          `${originalFile.name} convertido para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      } else {
        ...
        downloadFile(
          zipBlob,
          withZipSuffix(state.files[0]?.name || FILETYPE, 'convertido')
        );
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} arquivo(s) ${TOOL_NAME} convertido(s) para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      }
