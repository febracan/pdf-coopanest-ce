    ...
    img.onload = () => {
      ...
      canvas.toBlob((pngBlob) => {
        ...
        if (pngBlob) {
          ...
          reject(new Error(`Falha ao converter ${filename} em PNG`));
        }
      }, 'image/png');
    };
    img.onerror = () => {
      URL.revokeObjectURL(url);
      reject(new Error(`Falha ao carregar a imagem: ${filename}`));
    };
    img.src = url;
  });
}
...
async function convertCbzToPdf(file: File): Promise<Blob> {
  ...
  if (imageFiles.length > MAX_CBZ_PAGES) {
    throw new Error(
      `O arquivo compactado tem imagens demais (máx. ${MAX_CBZ_PAGES}).`
    );
  }
  let totalBytes = 0;
  ...
  for (const filename of imageFiles) {
    ...
    if (typeof declared === 'number' && declared > MAX_ENTRY_BYTES) {
      throw new Error('O arquivo compactado contém uma imagem grande demais.');
    }
    const imageData = await zipEntry.async('arraybuffer');
    totalBytes += imageData.byteLength;
    if (totalBytes > MAX_TOTAL_BYTES) {
      throw new Error(
        'O arquivo compactado fica grande demais ao ser descompactado.'
      );
    }
  ...
  const convertToPdf = async () => {
    try {
      if (state.files.length === 0) {
        showAlert(
          'Nenhum arquivo',
          `Selecione ao menos um arquivo ${TOOL_NAME}.`
        );
        return;
      }
      ...
      if (state.files.length === 1) {
        ...
        showLoader(`Convertendo ${originalFile.name}...`);
        ...
        showAlert(
          'Conversão concluída',
          `${originalFile.name} convertido em PDF com sucesso.`,
          'success',
          () => resetState()
        );
      } else {
        showLoader('Convertendo arquivos...');
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} arquivo(s) ${TOOL_NAME} convertido(s) em PDF com sucesso.`,
          'success',
          () => resetState()
        );
      }
      ...
      showAlert(
        'Erro',
        `Ocorreu um erro durante a conversão. Erro: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  };
