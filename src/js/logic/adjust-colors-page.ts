...
async function processAllPages(): Promise<void> {
  ...
  showLoader('Aplicando ajustes de cor...');
  ...
  try {
    ...
    showAlert(
      'Erro',
      'Falha ao aplicar os ajustes de cor. O arquivo pode estar corrompido.'
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = async (newFiles: FileList | null) => {
    ...
    try {
      ...
      if (!result) return;
      showLoader('Carregando pré-visualização...');
      files = [result.file];
      updateUI();
      pdfjsDoc = result.pdf;
      await renderPreview();
      ...
      console.error(e);
      showAlert('Erro', 'Falha ao carregar o PDF para pré-visualização.');
    } finally {
      hideLoader();
    }
  };
