...
async function processAllPages(): Promise<void> {
  ...
  showLoader('Aplicando efeito de digitalização...');
  ...
  try {
    ...
    for (let i = 1; i <= doc.numPages; i++) {
      showLoader(`Processando página ${i} de ${doc.numPages}...`);
    ...
    downloadFile(
      ...
      withPdfSuffix(files[0]?.name || 'document.pdf', 'digitalizado')
    );
    showAlert(
      'Sucesso',
      'Efeito de digitalização aplicado com sucesso!',
      'success',
      () => {
        resetState();
      }
    ...
    showAlert(
      'Erro',
      'Falha ao aplicar o efeito de digitalização. O arquivo pode estar corrompido.'
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = async (newFiles: FileList | null) => {
    ...
    try {
      ...
      console.error(e);
      showAlert('Erro', 'Falha ao carregar o PDF para a prévia.');
    } finally {
      hideLoader();
    }
  };
