...
export async function parseMsgFile(file: File): Promise<ParsedEmail> {
  ...
  return {
    subject: msgData.subject || '(Sem assunto)',
    from,
    to,
    cc,
    bcc,
...
export function renderEmailToHtml(
  ...
  const attachmentHtml =
    ...
    <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #cccccc;">
      <p style="margin: 0 0 8px 0; font-size: 11px; color: #666; font-weight: 600;">Anexos (${email.attachments.length})</p>
  ...
  if (includeCcBcc) {
    ...
    if (email.bcc.length > 0) {
      ccBccHtml += `
      <div style="margin-bottom: 8px;">
        <span style="font-weight: 600; color: #666; margin-right: 8px;">Cco:</span>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 40px 20px; background: #fff;">
  <div style="border-bottom: 2px solid #999999; padding-bottom: 20px; margin-bottom: 30px;">
    ...
    <div style="font-size: 12px; color: #555;">
      <div style="margin-bottom: 8px;">
        <span style="font-weight: 600; color: #666; margin-right: 8px;">De:</span>
        <span style="color: #333;">${escapeHtml(email.from)}</span>
      </div>
      <div style="margin-bottom: 8px;">
        <span style="font-weight: 600; color: #666; margin-right: 8px;">Para:</span>
        <span style="color: #333;">${escapeHtml(email.to.join(', ') || 'Desconhecido')}</span>
      </div>
      ${ccBccHtml}
      <div style="margin-bottom: 8px;">
        <span style="font-weight: 600; color: #666; margin-right: 8px;">Data:</span>
