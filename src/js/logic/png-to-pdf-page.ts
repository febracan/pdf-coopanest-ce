  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        async (jpegBlob) => {
          if (!jpegBlob) {
            return reject(new Error('Falha na conversão do canvas (toBlob).'));
          }
    ...
    img.onerror = () => {
      ...
      reject(
        new Error(
          'Não foi possível carregar o arquivo como imagem. Ele pode estar corrompido.'
        )
      );
    };
...
setupImageToPdfPage({
  isValid: isPng,
  invalidMessage:
    'Alguns arquivos foram ignorados. Somente imagens PNG são permitidas.',
  emptyMessage: 'Selecione pelo menos um arquivo PNG.',
  suffix: 'convertido',
  optionsSelector: '#jpg-to-pdf-options',
  loadingMessage: 'Criando PDF a partir dos PNGs...',
  convert: buildPdf,
});
