  ...
  const extractForAI = async () => {
    try {
      ...
      if (total === 1) {
        ...
        showLoader(`Extraindo ${file.name} para IA...`);
        ...
        showAlert(
          'Extração concluída',
          `PDF extraído com sucesso para uso com IA/LLM.`,
          'success',
          () => resetState()
        );
      } else {
        ...
        for (let fi = 0; fi < state.files.length; fi++) {
          try {
            ...
            showLoader(
              `Extraindo ${file.name} para IA (${completed + 1}/${total})...`
            );
        ...
        if (failed === 0) {
          showAlert(
            'Extração concluída',
            `${completed} PDF(s) extraído(s) com sucesso para uso com IA/LLM.`,
            'success',
            () => resetState()
          );
        } else {
          showAlert(
            'Extração parcial',
            `${completed} PDF(s) extraído(s), ${failed} com falha.`,
            'warning',
            () => resetState()
          );
        }
