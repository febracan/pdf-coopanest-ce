        ...
        <div class="bg-gray-700 p-3 rounded-lg border border-gray-600 hover:border-indigo-500 transition-colors">
            <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-gray-400 text-sm">${fileSize} • ${pageCount} ${pageCount !== 1 ? 'páginas' : 'página'}</p>
                </div>
                <button id="remove-file" class="text-red-400 hover:text-red-300 p-2 flex-shrink-0 ml-2" title="Remover arquivo">
...
async function handleFileUpload(file: File) {
  if (!file || file.type !== 'application/pdf') {
    showAlert('Erro', 'Envie um arquivo PDF válido.');
    return;
  }
  try {
...
async function detectBlankPages() {
  if (!pageState.pdfDoc || !pageState.file)
    return showAlert('Erro', 'Envie um PDF primeiro.');
  ...
  showLoader('Detectando páginas em branco...');
  try {
    ...
    if (pageState.detectedBlankPages.length === 0) {
      showAlert('Informação', 'Nenhuma página em branco detectada neste PDF.');
      hideLoader();
      return;
    }
    ...
    console.error(e);
    showAlert('Erro', 'Não foi possível detectar páginas em branco.');
    hideLoader();
  }
}
...
function updatePreviewPanel() {
  ...
  previewInfo.textContent = `${pageState.detectedBlankPages.length} página(s) em branco encontrada(s). Clique em uma página para desmarcá-la.`;
  ...
  pageState.detectedBlankPages.forEach((pageIndex) => {
    ...
    div.innerHTML = `
            <div class="relative">
                <img src="${thumbnail}" alt="Página ${pageIndex + 1}" class="rounded-md shadow-md max-w-full h-auto">
...
async function processRemoveBlankPages() {
  ...
  if (selectedPages.length === 0) {
    showAlert('Informação', 'Nenhuma página selecionada para remoção.');
    return;
  }

  showLoader(`Removendo ${selectedPages.length} página(s) em branco...`);
  try {
    ...
    showAlert(
      'Sucesso',
      `${selectedPages.length} página(s) em branco removida(s) com sucesso!`,
      'success',
      resetState
    );
  } catch (e) {
