...
async function convert() {
  if (files.length === 0) {
    showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo TIFF.');
    return;
  }
  ...
  try {
    ...
    showAlert(
      'Erro',
      'Falha ao converter TIFF para PDF. Um dos arquivos pode ser inválido.'
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        'Arquivos inválidos',
        'Alguns arquivos foram ignorados. Apenas arquivos TIFF são permitidos.'
      );
    }
