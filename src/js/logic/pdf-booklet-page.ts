...
function resetState() {
  ...
  if (previewArea)
    previewArea.innerHTML =
      '<p class="text-gray-400 text-center py-8">Envie um PDF e clique em "Gerar prévia" para ver o layout do livreto</p>';
...
async function generatePreview() {
  if (!pageState.pdfDoc || !pageState.pdfjsDoc) {
    showAlert('Erro', 'Carregue um PDF primeiro.');
    return;
  }
  ...
  previewArea.innerHTML =
    '<p class="text-gray-400 text-center py-4">Gerando prévia...</p>';
  ...
  previewArea.innerHTML = `<p class="text-indigo-400 text-sm mb-4 text-center">${totalPages} páginas → ${numSheets} folhas de saída</p>`;
  ...
  for (let sheetIndex = 0; sheetIndex < numSheets; sheetIndex++) {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (exists) {
          ...
          ctx.fillText('(em branco)', x + slotWidth / 2, y + slotHeight / 2);
        }
      }
    }
    ...
    const sideLabel = isBookletMode ? (isFront ? 'Frente' : 'Verso') : '';
    ctx.fillText(
      `Folha ${Math.floor(sheetIndex / (isBookletMode ? 2 : 1)) + 1} ${sideLabel}`,
      canvasWidth - 6,
      4
    );
...
async function createBooklet() {
  ...
  try {
    ...
    showAlert(
      'Sucesso',
      `Livreto criado com ${numSheets} folhas!`,
      'success',
      function () {
        resetState();
      }
    ...
    console.error(e);
    showAlert('Erro', 'Ocorreu um erro ao criar o livreto.');
  } finally {
    hideLoader();
  }
}
