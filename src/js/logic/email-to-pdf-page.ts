  ...
  const convertToPdf = async () => {
    try {
      if (state.files.length === 0) {
        showAlert(
          'Nenhum arquivo',
          `Selecione ao menos um arquivo ${TOOL_NAME} (.eml ou .msg).`
        );
        return;
      }
      ...
      showLoader('Carregando mecanismo de PDF...');
      const pymupdf = await loadPyMuPDF();

      if (state.files.length === 1) {
        const originalFile = state.files[0];
        showLoader(`Analisando ${originalFile.name}...`);
        ...
        showLoader('Convertendo e-mails...');
