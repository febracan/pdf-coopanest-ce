...
async function updatePdfDisplay(): Promise<void> {
  ...
  metaSpan.textContent = `${formatBytes(state.pdfFile.size)} • Carregando páginas...`;
  ...
  try {
    if (state.pdfBytes) {
      ...
      metaSpan.textContent = `${formatBytes(state.pdfFile.size)} • ${pdfDoc.numPages} páginas`;
    }
  } catch (error) {
...
async function processTimestamp(): Promise<void> {
  ...
  showLoader('Aplicando carimbo de data/hora...');
  ...
  try {
    ...
    showAlert(
      'Falha no carimbo de data/hora',
      `Falha ao aplicar carimbo de data/hora no PDF: ${message}\n\nTente um servidor TSA diferente ou verifique sua conexão com a internet.`
    );
  } finally {
    hideLoader();
  }
