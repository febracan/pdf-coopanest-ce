...
async function changePermissions() {
  ...
  if (newUserPassword && !newOwnerPassword) {
    showAlert(
      'Senha de proprietário obrigatória',
      'É necessária uma senha de proprietário ao definir permissões'
    );
    return;
  }
  ...
  try {
    ...
    if (loaderText) loaderText.textContent = 'Processando permissões do PDF...';
    ...
    try {
      ...
      throw new Error(
        'Falha no processamento: ' + errorMsg || 'Erro desconhecido',
        {
          cause: qpdfError,
        }
      );
    ...
    if (!outputFile || outputFile.length === 0) {
      throw new Error('O processamento resultou em um arquivo vazio.');
    }
    ...
    if (!shouldEncrypt) {
      successMessage =
        'PDF descriptografado com sucesso! Toda a criptografia e restrições foram removidas.';
    }
    ...
    if (errorMessage === 'INVALID_PASSWORD') {
      showAlert(
        'Senha incorreta',
        'A senha atual que você digitou está incorreta. Tente novamente.'
      );
    } else if (errorMessage === 'PASSWORD_REQUIRED') {
      showAlert(
        'Senha obrigatória',
        'Este PDF é protegido por senha. Digite a senha atual para continuar.'
      );
    } else {
      showAlert(
        'Falha no processamento',
        `Ocorreu um erro: ${errorMessage || 'O PDF pode estar corrompido ou protegido por senha.'}`
      );
    }
  } finally {
    try {
