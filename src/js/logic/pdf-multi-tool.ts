    ...
    for (const file of files) {
      ...
      try {
        ...
        try {
          ...
          const loadingText = document.getElementById('loading-text');
          if (loadingText)
            loadingText.textContent = `Reparando ${file.name}...`;
...
async function handleInsertPdf(e: Event) {
  ...
  try {
    ...
    showModal(
      'Erro',
      'Falha ao inserir o PDF. O arquivo pode estar corrompido.',
      'error'
    );
  }
...
function bulkRotate(delta: number) {
  if (selectedPages.size === 0) {
    showModal('Nenhuma seleção', 'Selecione páginas para girar.', 'info');
    return;
  }
...
function bulkDelete() {
  if (selectedPages.size === 0) {
    showModal('Nenhuma seleção', 'Selecione páginas para excluir.', 'info');
    return;
  }
...
function bulkDuplicate() {
  if (selectedPages.size === 0) {
    showModal('Nenhuma seleção', 'Selecione páginas para duplicar.', 'info');
    return;
  }
  const indices = Array.from(selectedPages).sort((a, b) => b - a);
...
function bulkSplit() {
  if (selectedPages.size === 0) {
    showModal(
      'Nenhuma seleção',
      'Selecione páginas para marcar para divisão.',
      'info'
    );
    return;
  }
...
async function downloadAll() {
  if (allPages.length === 0) {
    showModal('Nenhuma página', 'Envie os PDFs primeiro.', 'info');
    return;
  }
...
async function downloadSplitPdfs() {
  try {
    ...
    showModal(
      'Sucesso',
      `${segments.length} arquivos PDF baixados em um arquivo ZIP.`,
      'success'
    );
  } catch (e) {
    console.error('Failed to create split PDFs:', e);
    showModal('Erro', 'Falha ao criar os PDFs divididos.', 'error');
  } finally {
...
async function downloadPagesAsPdf(indices: number[], filename: string) {
  try {
    ...
    showModal('Erro', 'Falha ao criar o PDF.', 'error');
  }
}
...
function updatePageDisplay() {
  ...
  allPages.forEach((pageData, index) => {
    ...
    if (card) {
      ...
      if (info) info.textContent = `Página ${index + 1} `;
