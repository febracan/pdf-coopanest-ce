  ...
  const convertToPdfA = async () => {
    ...
    try {
      ...
      if (state.files.length === 1) {
        ...
        showLoader('Inicializando o Ghostscript...');
        ...
        showAlert(
          'Conversão concluída',
          `${originalFile.name} convertido para ${level} com sucesso.`,
          'success',
          () => resetState()
        );
      } else {
        showLoader('Convertendo vários PDFs para PDF/A...');
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} PDF(s) convertido(s) para ${level} com sucesso.`,
          'success',
          () => resetState()
        );
      }
