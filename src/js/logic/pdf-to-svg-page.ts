...
async function convert() {
  ...
  try {
    ...
    showLoader('Convertendo para SVG...');
    ...
    if (isSingleFile) {
      ...
      if (pageCount === 1) {
        ...
        showAlert(
          'Sucesso',
          'PDF convertido para SVG com sucesso!',
          'success',
          () => resetState()
        );
      } else {
        ...
        for (let i = 0; i < pageCount; i++) {
          showLoader(`Convertendo página ${i + 1} de ${pageCount}...`);
        ...
        showAlert(
          'Sucesso',
          `${pageCount} páginas convertidas para SVG!`,
          'success',
          () => resetState()
        );
      }
      ...
      for (let f = 0; f < files.length; f++) {
        const file = files[f];
        showLoader(`Processando arquivo ${f + 1} de ${files.length}...`);
        ...
        for (let i = 0; i < pageCount; i++) {
          showLoader(
            `Arquivo ${f + 1}/${files.length}: Página ${i + 1}/${pageCount}`
          );
          const page = doc.getPage(i);
          const svgContent = page.toSvg();
          const fileName =
      ...
      showAlert(
        'Sucesso',
        `${files.length} arquivos (${totalPages} páginas) convertidos para SVG!`,
        'success',
        () => resetState()
      );
    }
    ...
    showAlert('Erro', `Falha ao converter PDF para SVG. ${message}`);
  } finally {
    hideLoader();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = (newFiles: FileList | null, replace = false) => {
    ...
    if (validFiles.length === 0) {
      showAlert('Arquivos inválidos', 'Envie arquivos PDF.');
      return;
    }
