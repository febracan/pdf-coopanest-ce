...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(deleteState.file.size)} • ${deleteState.totalPages} páginas`;
...
async function deletePages() {
  if (deleteState.pagesToDelete.size === 0) {
    showAlert('Nenhuma página', 'Selecione as páginas a excluir.');
    return;
  }

  if (deleteState.pagesToDelete.size >= deleteState.totalPages) {
    showAlert('Erro', 'Não é possível excluir todas as páginas.');
    return;
  }
  ...
  try {
    ...
    showAlert(
      'Sucesso',
      `${deleteState.pagesToDelete.size} página(s) excluída(s) com sucesso!`,
      'success',
      () => resetState()
    );
  } catch (error) {
    ...
    hideLoader();
    showAlert('Erro', 'Falha ao excluir as páginas.');
  }
}
