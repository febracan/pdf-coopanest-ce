...
async function flattenPdf() {
  ...
  try {
    if (pageState.files.length === 1) {
      ...
      if (loaderText) loaderText.textContent = 'Achatando PDF...';
      ...
      if (loaderModal) loaderModal.classList.remove('hidden');
      if (loaderText) loaderText.textContent = 'Achatando vários PDFs...';
      ...
      for (let i = 0; i < pageState.files.length; i++) {
        ...
        if (loaderText)
          loaderText.textContent = `Achatando ${i + 1}/${pageState.files.length}: ${file.name}...`;
