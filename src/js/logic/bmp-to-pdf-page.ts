...
async function convert() {
  if (files.length === 0) {
    showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo BMP.');
    return;
  }
  showLoader('Convertendo BMP em PDF...');
  try {
    ...
    showAlert(
      'Erro',
      'Falha ao converter BMP em PDF. Um dos arquivos pode estar inválido.'
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        'Arquivos inválidos',
        'Alguns arquivos foram ignorados. Somente arquivos BMP são permitidos.'
      );
    }
