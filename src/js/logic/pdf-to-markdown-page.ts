...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const convert = async () => {
    try {
      ...
      showLoader('Carregando conversor de PDF...');
      ...
      if (state.files.length === 1) {
        ...
        showAlert(
          'Conversão concluída',
          `${file.name} convertido para Markdown com sucesso.`,
          'success',
          () => resetState()
        );
      } else {
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} PDF(s) convertido(s) para Markdown com sucesso.`,
          'success',
          () => resetState()
        );
      }
