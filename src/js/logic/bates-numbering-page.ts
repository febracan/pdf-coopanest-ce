...
const STYLE_PRESETS: Record<string, StylePreset> = {
  'full-6': {
    template: 'Anexo [FILE] Caso XYZ [BATES] Página [PAGE]',
    padding: 6,
  },
  'full-5': {
    template: 'Anexo [FILE] Caso XYZ [BATES] Página [PAGE]',
  ...
  'no-page-6': { template: 'Anexo [FILE] Caso XYZ [BATES]', padding: 6 },
  'no-page-5': { template: 'Anexo [FILE] Caso XYZ [BATES]', padding: 5 },
  'no-page-4': { template: 'Anexo [FILE] Caso XYZ [BATES]', padding: 4 },
  'no-page-3': { template: 'Anexo [FILE] Caso XYZ [BATES]', padding: 3 },
  'no-page-0': { template: 'Anexo [FILE] Caso XYZ [BATES]', padding: 0 },
  'case-6': { template: 'Caso XYZ [BATES]', padding: 6 },
  'case-5': { template: 'Caso XYZ [BATES]', padding: 5 },
  'case-4': { template: 'Caso XYZ [BATES]', padding: 4 },
  'case-3': { template: 'Caso XYZ [BATES]', padding: 3 },
  'case-0': { template: 'Caso XYZ [BATES]', padding: 0 },
...
async function handleFiles(fileList: FileList) {
  ...
  try {
    ...
    if (files.length === 0) {
      showAlert('Arquivo inválido', 'Envie arquivos PDF válidos.');
      return;
    }
...
function renderFileList() {
  ...
  files.forEach((entry, index) => {
    ...
    metaSpan.textContent = `${formatBytes(entry.file.size)} \u2022 ${entry.pageCount} p\u00e1ginas`;
  ...
  summary.textContent = `${files.length} arquivo${files.length !== 1 ? 's' : ''} \u2022 ${totalPages} p\u00e1ginas no total`;
...
function updatePreview() {
  ...
  if (files.length === 0) {
    lines.push(
      formatBatesText(template, batesStart, 1, fileStart, 'documento', padding)
    );
    lines.push(
      formatBatesText(
        template,
    ...
    for (const entry of files) {
      ...
      lines.push(
        `Arquivo ${fileCounter}, Página 1: ${formatBatesText(template, batesCounter, 1, fileCounter, name, padding)}`
      );
      if (entry.pageCount > 1) {
        lines.push(
          `Arquivo ${fileCounter}, Página 2: ${formatBatesText(template, batesCounter + 1, 2, fileCounter, name, padding)}`
        );
      }
    ...
    lines.push('...');
    lines.push(
      `Arquivo ${fileStart + files.length - 1}, Página ${lastEntry.pageCount}: ${formatBatesText(template, lastBates, lastEntry.pageCount, fileStart + files.length - 1, lastName, padding)}`
    );
  }
...
async function applyBatesNumbers() {
  if (files.length === 0) {
    showAlert('Erro', 'Envie ao menos um arquivo PDF.');
    return;
  }

  showLoader('Aplicando numeração Bates...');
  try {
    const template = (
      document.getElementById('bates-template') as HTMLInputElement
    ).value;
    ...
    showAlert(
      'Sucesso',
      `Numeração Bates aplicada com sucesso! (${batesStart} até ${batesCounter - 1})`,
      'success',
      () => {
        resetState();
      }
    ...
    console.error(e);
    showAlert('Erro', 'Falha ao aplicar a numeração Bates.');
  } finally {
    hideLoader();
  }
}
