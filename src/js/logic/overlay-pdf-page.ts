...
async function processOverlay() {
  if (!pageState.baseFile || !pageState.overlayFile) {
    showAlert(
      'Arquivos ausentes',
      'Envie tanto um PDF base quanto um PDF de sobreposição/subposição.'
    );
    return;
  }
  ...
  if (loaderText)
    loaderText.textContent = 'Inicializando o mecanismo de PDF...';
  ...
  try {
    ...
    if (loaderText) loaderText.textContent = 'Lendo arquivos...';
    ...
    if (loaderText)
      loaderText.textContent = `Aplicando ${mode.replace('--', '')}...`;
    ...
    showAlert(
      'Sucesso',
      `PDF ${modeLabel} aplicado com sucesso.`,
      'success',
      () => {
        resetState();
      }
    ...
    showAlert(
      ...
      `Ocorreu um erro: ${error instanceof Error ? error.message : 'Unknown error'}.`
    );
  } finally {
    try {
      if (qpdf?.FS) {
