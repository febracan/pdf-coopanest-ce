...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const convertToPdf = async () => {
    try {
      ...
      if (state.files.length === 0) {
        showAlert('Nenhum arquivo', 'Selecione pelo menos um arquivo CSV.');
        hideLoader();
        return;
      }
      ...
      if (state.files.length === 1) {
        ...
        showLoader('Preparando conversão...');
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} arquivo(s) CSV convertido(s) para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      }
