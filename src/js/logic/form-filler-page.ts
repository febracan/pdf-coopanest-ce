...
// UI helpers
function showLoader(message: string = 'Processando...') {
...
async function setupFormViewer() {
  ...
  showLoader('Carregando formulário PDF...');
  ...
  try {
    ...
    showAlert('Erro', 'Falha ao carregar o visualizador de formulário PDF.');
    hideLoader();
  }
}
...
async function processAndDownloadForm() {
  if (!viewerIframe || !viewerReady) {
    showAlert(
      'Visualizador não está pronto',
      'Aguarde o formulário terminar de carregar.'
    );
    return;
  }
  ...
  try {
    ...
    if (!viewerWindow) {
      ...
      showAlert(
        'Baixar',
        'Use o botão Baixar na barra de ferramentas do visualizador de PDF acima.'
      );
      return;
    }
    ...
    showAlert(
      'Baixar',
      'Não é possível acessar os controles do visualizador. Use o botão Baixar na barra de ferramentas do visualizador de PDF acima.'
    );
  }
}
