...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(extractState.file.size)} • ${extractState.totalPages} páginas`;
...
async function extractPages() {
  ...
  if (!pagesInput || !pagesInput.value.trim()) {
    showAlert('Nenhuma página', 'Digite os números das páginas a extrair.');
    return;
  }
  ...
  if (pagesToExtract.length === 0) {
    showAlert(
      'Páginas inválidas',
      'Nenhum número de página válido encontrado.'
    );
    return;
  }
  ...
  try {
    ...
    showAlert(
      'Sucesso',
      `${pagesToExtract.length} página(s) extraída(s) com sucesso!`,
      'success',
      () => {
        resetState();
      }
    ...
    hideLoader();
    showAlert('Erro', 'Falha ao extrair as páginas.');
  }
}
