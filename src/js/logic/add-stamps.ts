...
if (saveStampedBtn) {
  saveStampedBtn.addEventListener('click', () => {
    if (!viewerIframe) {
      alert(
        'O visualizador não está pronto. Envie um PDF e aguarde ele terminar de carregar.'
      );
      return;
    }
    ...
    try {
      ...
      alert(
        'Não foi possível acessar o exportador do PDF carimbado. Como alternativa, use o botão Exportar → PDF na barra de ferramentas do visualizador.'
      );
    } catch (e) {
      console.error('Failed to trigger stamped PDF export:', e);
      alert(
        'Não foi possível exportar o PDF carimbado. Como alternativa, use o botão Exportar → PDF na barra de ferramentas do visualizador.'
      );
    }
  });
}
