...
function applyCustomOrder() {
  ...
  if (!orderString) {
    showAlert('Ordem inválida', 'Informe uma ordem de páginas.');
    return;
  }
  ...
  if (!validNumbers) {
    showAlert('Números de página inválidos', `Informe números positivos.`);
    return;
  }

  if (newOrder.length !== currentGridCount) {
    showAlert(
      'Contagem de páginas incorreta',
      `O número de páginas informado (${newOrder.length}) não corresponde ao número atual de páginas no documento (${currentGridCount}). Forneça uma ordenação completa para todas as páginas.`
    );
    return;
  }
  ...
  if (uniqueNumbers.size !== newOrder.length) {
    showAlert(
      ...
      'Certifique-se de que todos os números de página na ordem sejam únicos.'
    );
    return;
  }
  ...
  if (
    ...
    showAlert(
      'Ordem de páginas inválida',
      'A ordem de páginas especificada está incompleta ou contém números de página inválidos. Certifique-se de fornecer uma nova posição para cada página original.'
    );
    return;
  }
  ...
  showAlert('Sucesso', 'As páginas foram reordenadas.', 'success');
}
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(organizeState.file.size)} • ${organizeState.totalPages} páginas`;
...
async function saveChanges() {
  showLoader('Montando o novo PDF...');
  ...
  try {
    ...
    if (finalIndices.length === 0) {
      showAlert('Erro', 'Nenhuma página válida para salvar.');
      return;
    }
    ...
    hideLoader();
    showAlert('Sucesso', 'PDF organizado com sucesso!', 'success', () =>
      resetState()
    );
  } catch (error) {
