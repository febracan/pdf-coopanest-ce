...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (state.files.length > 0) {
      ...
      for (let index = 0; index < state.files.length; index++) {
        ...
        try {
          ...
          metaSpan.textContent = `${formatBytes(file.size)} • ${pdfDoc.numPages} páginas`;
        } catch {
          metaSpan.textContent = `${formatBytes(file.size)} • Não foi possível carregar a contagem de páginas`;
        }
      }
  ...
  const extract = async () => {
    try {
      if (state.files.length === 0) {
        showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo PDF.');
        return;
      }

      const decryptedFiles = await batchDecryptIfNeeded(state.files);
      showLoader('Carregando processador de PDF...');
      ...
      for (let i = 0; i < state.files.length; i++) {
        ...
        showLoader(`Extraindo imagens de ${file.name}...`);
      ...
      if (extractedImages.length === 0) {
        showAlert(
          ...
          'Nenhuma imagem incorporada foi encontrada no(s) PDF(s) selecionado(s).'
        );
      } else {
        displayImages();
        showAlert(
          'Extração concluída',
          `${extractedImages.length} imagem(ns) encontrada(s) em ${state.files.length} PDF(s).`,
          'success'
        );
      }
    } catch (e: unknown) {
      ...
      showAlert(
        'Erro',
        `Ocorreu um erro durante a extração. Erro: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  };
