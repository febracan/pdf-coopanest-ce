  ...
  const rasterize = async () => {
    try {
      ...
      if (total === 1) {
        ...
        showLoader(`Rasterizando ${file.name}...`);
        ...
        showAlert(
          ...
          `PDF rasterizado com sucesso a ${dpi} DPI.`,
          'success',
          () => resetState()
        );
      } else {
        ...
        for (let fi = 0; fi < state.files.length; fi++) {
          try {
            ...
            showLoader(
              `Rasterizando ${file.name} (${completed + 1}/${total})...`
            );
        ...
        if (failed === 0) {
          showAlert(
            'Rasterização concluída',
            `${completed} PDF(s) rasterizado(s) com sucesso a ${dpi} DPI.`,
            'success',
            () => resetState()
          );
        } else {
          showAlert(
            'Rasterização parcial',
            `${completed} PDF(s) rasterizado(s), ${failed} com falha.`,
            'warning',
            () => resetState()
          );
        }
      ...
      showAlert(
        'Erro',
        `Ocorreu um erro durante a rasterização. Erro: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  };
