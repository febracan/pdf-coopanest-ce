...
async function updateUI() {
  ...
  if (pageState.file) {
    ...
    try {
      ...
      if (pagePositionHint) {
        pagePositionHint.textContent = `Digite 0 para inserir no início, ou ${pageCount} para inserir no fim.`;
      }
...
async function addBlankPages() {
  ...
  if (isNaN(position) || position < 0 || position > totalPages) {
    showAlert('Entrada inválida', `Digite um número entre 0 e ${totalPages}.`);
    return;
  }
  ...
  if (isNaN(insertCount) || insertCount < 1) {
    showAlert(
      'Entrada inválida',
      'Digite um número válido de páginas (1 ou mais).'
    );
    return;
  }

  showLoader(
    `Adicionando ${insertCount} página${insertCount > 1 ? 's' : ''} em branco...`
  );
  ...
  try {
    ...
    showAlert(
      'Sucesso',
      `${insertCount} página${insertCount > 1 ? 's' : ''} em branco adicionada${insertCount > 1 ? 's' : ''} com sucesso!`,
      'success',
      function () {
        resetState();
      }
    ...
    showAlert(
      'Erro',
      `Não foi possível adicionar a${insertCount > 1 ? 's' : ''} página${insertCount > 1 ? 's' : ''} em branco.`
    );
  } finally {
    hideLoader();
  }
