...
async function linearizePdf() {
  ...
  if (!pdfFiles || pdfFiles.length === 0) {
    showAlert('Nenhum arquivo PDF', 'Envie pelo menos um arquivo PDF.');
    return;
  }
  ...
  if (loaderText)
    loaderText.textContent =
      'Otimizando PDFs para visualização web (linearizando)...';
  ...
  try {
    ...
    for (let i = 0; i < pdfFiles.length; i++) {
      ...
      if (loaderText)
        loaderText.textContent = `Otimizando ${file.name} (${i + 1}/${pdfFiles.length})...`;
    ...
    if (successCount === 0) {
      throw new Error('Nenhum arquivo PDF pôde ser linearizado.');
    }
    ...
    if (errorCount > 0) {
      alertMessage += ` ${errorCount} arquivo(s) falhou(aram).`;
    }
