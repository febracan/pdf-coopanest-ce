...
async function encryptPdf() {
  ...
  if (!userPassword) {
    showAlert('Entrada obrigatória', 'Digite uma senha de usuário.');
    return;
  }
  ...
  try {
    ...
    if (loaderText) loaderText.textContent = 'Inicializando criptografia...';
    ...
    if (loaderText)
      loaderText.textContent = 'Criptografando PDF com AES de 256 bits...';
    ...
    let successMessage =
      'PDF criptografado com sucesso usando AES de 256 bits!';
    if (!hasDistinctOwnerPassword) {
      successMessage +=
        ' Observação: sem uma senha de proprietário separada, o PDF não tem restrições de uso.';
    }
    ...
    showAlert(
      'Falha na criptografia',
      `Ocorreu um erro: ${error instanceof Error ? error.message : 'O PDF pode estar corrompido.'}`
    );
  } finally {
    try {
      if (qpdf?.FS) {
