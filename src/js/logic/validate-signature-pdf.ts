...
export async function validateSignature(
  ...
  const result: SignatureValidationResult = {
    ...
    isSelfSigned: false,
    isTrusted: false,
    algorithms: { digest: 'Desconhecido', signature: 'Desconhecido' },
    serialNumber: '',
  ...
  try {
    ...
    if (!p7.certificates || p7.certificates.length === 0) {
      result.errorMessage = 'Nenhum certificado encontrado na assinatura';
      return result;
    }
    ...
    result.signerName = (subjectCN?.value as string) ?? 'Desconhecido';
    result.signerOrg = subjectO?.value as string | undefined;
    result.signerEmail = subjectE?.value as string | undefined;
    result.issuer = (issuerCN?.value as string) ?? 'Desconhecido';
    ...
    if (verification.status === 'unsupported') {
      ...
      result.errorMessage =
        verification.reason || 'Falha na verificação criptográfica';
    }
    ...
    result.errorMessage =
      e instanceof Error ? e.message : 'Falha ao analisar a assinatura';
  }
...
function getDigestAlgorithmName(oid: string): string {
  ...
  return digestAlgorithms[oid] || oid || 'Desconhecido';
}
...
function getSignatureAlgorithmName(oid: string): string {
  ...
  return signatureAlgorithms[oid] || oid || 'Desconhecido';
}
...
function detectSigScheme(
  ...
  if (!signatureAlgorithmArr || signatureAlgorithmArr.length === 0) {
    return { unsupported: 'signatureAlgorithm ausente' };
  }
  const oidNode = signatureAlgorithmArr[0];
  if (!oidNode || oidNode.type !== forge.asn1.Type.OID) {
    return { unsupported: 'signatureAlgorithm malformado' };
  }
  const oid = forge.asn1.derToOid(oidNode.value as string);
  ...
  switch (oid) {
    case '1.2.840.113549.1.1.1':
      return implicitHash
        ...
        : { unsupported: `OID de digest não suportado ${digestOid}` };
    ...
    default:
      return {
        unsupported: `OID de algoritmo de assinatura não suportado ${oid}`,
      };
  }
}
...
async function verifyViaWebCrypto(
  ...
  if (!subtle) {
    return {
      status: 'unsupported',
      reason: 'API Web Crypto não disponível neste contexto',
    };
  }
  ...
  try {
    if (scheme.kind === 'rsa-pss') {
      ...
      return ok
        ...
        : {
            status: 'failed',
            reason:
              'A assinatura RSA-PSS não confere com a chave pública do signatário',
          };
    }
    ...
    if (scheme.kind === 'ecdsa') {
      ...
      if (!curve) {
        return {
          status: 'unsupported',
          reason: 'Curva ECDSA não suportada no certificado do signatário',
        };
      }
      ...
      if (!p1363) {
        return {
          status: 'failed',
          reason: 'Assinatura ECDSA malformada (não foi possível analisar r,s)',
        };
      }
      const key = await subtle.importKey(
        'spki',
      ...
      return ok
        ...
        : {
            status: 'failed',
            reason:
              'A assinatura ECDSA não confere com a chave pública do signatário',
          };
    }
    ...
    if (scheme.kind === 'rsa-pkcs1') {
      ...
      return ok
        ...
        : {
            status: 'failed',
            reason:
              'A assinatura RSA-PKCS1 não confere com a chave pública do signatário',
          };
    }

    return {
      status: 'unsupported',
      reason: `Esquema de assinatura ${scheme.kind} não implementado`,
    };
  } catch (e) {
    return {
      status: 'unsupported',
      reason:
        'Falha na importação/verificação do Web Crypto: ' +
...
async function performCryptoVerification(
  ...
  if (!fields) {
    return {
      status: 'failed',
      reason: 'Não foi possível analisar as informações do signatário',
    };
  }
  if (byteRange.length !== 4) {
    return { status: 'failed', reason: 'ByteRange malformado' };
  }
  ...
  if (!md) {
    return {
      status: 'unsupported',
      reason: `OID de digest não suportado ${fields.digestOid}`,
    };
  }
  ...
  if (
    ...
    return { status: 'failed', reason: 'ByteRange fora dos limites' };
  }
  ...
  if (!signatureBytes) {
    return { status: 'failed', reason: 'Bytes de assinatura vazios' };
  }
  ...
  if (authAttrs) {
    ...
    if (messageDigestAttrValue === null) {
      return {
        status: 'failed',
        reason: 'Atributo messageDigest ausente nos atributos autenticados',
      };
    }
    if (messageDigestAttrValue !== contentHashBytes) {
      return {
        status: 'failed',
        reason:
          'O hash do conteúdo não corresponde ao atributo messageDigest — o PDF foi modificado após a assinatura',
      };
    }
  ...
  if (!spkiDer) {
    return {
      status: 'unsupported',
      reason: 'Não foi possível extrair a chave pública do signatário',
    };
  }
