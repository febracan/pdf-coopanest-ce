...
function handleFiles(newFiles: FileList) {
  ...
  if (validFiles.length < newFiles.length) {
    showAlert(
      'Arquivos inválidos',
      'Alguns arquivos foram ignorados. Apenas arquivos PDF são permitidos.'
    );
  }
...
async function extractText() {
  ...
  try {
    ...
    showLoader('Extraindo texto...');

    if (files.length === 1) {
      const file = files[0];
      showLoader(`Extraindo texto de ${file.name}...`);
      ...
      hideLoader();
      showAlert('Sucesso', 'Texto extraído com sucesso!', 'success', () => {
        resetState();
      });
    } else {
      showLoader('Extraindo texto de vários arquivos...');
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        showLoader(
          `Extraindo texto do arquivo ${i + 1}/${files.length}: ${file.name}...`
        );
      ...
      showAlert(
        'Sucesso',
        `Texto extraído de ${files.length} arquivos PDF!`,
        'success',
        () => {
          resetState();
        }
    ...
    showAlert(
      'Erro de extração',
      e instanceof Error ? e.message : 'Falha ao extrair texto do PDF.'
    );
  }
}
