...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(cropperState.file.size)} • ${cropperState.pdfDoc?.numPages || 0} páginas`;
...
async function displayPageAsImage(num: number) {
  showLoader(`Renderizando página ${num}...`);
  ...
  try {
    ...
    showAlert('Erro', 'Falha ao renderizar a página.');
    hideLoader();
  }
}
...
function updatePageInfo() {
  ...
  if (pageInfo)
    pageInfo.textContent = `Página ${cropperState.currentPageNum} de ${cropperState.pdfDoc.numPages}`;
}
...
async function performCrop() {
  ...
  if (isApplyToAll) {
    ...
    if (!currentCrop) {
      showAlert(
        'Nenhuma área de recorte',
        'Selecione primeiro uma área para recortar.'
      );
      return;
    }
  ...
  if (Object.keys(finalCropData).length === 0) {
    showAlert(
      'Nenhuma área de recorte',
      'Selecione uma área em pelo menos uma página para recortar.'
    );
    return;
  }
