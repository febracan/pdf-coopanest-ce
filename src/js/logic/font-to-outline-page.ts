...
async function processFiles() {
  ...
  try {
    if (pageState.files.length === 1) {
      ...
      if (loaderText)
        loaderText.textContent = 'Convertendo fontes em contornos...';
      ...
      if (loaderModal) loaderModal.classList.remove('hidden');
      if (loaderText) loaderText.textContent = 'Processando vários PDFs...';
      ...
      for (let i = 0; i < pageState.files.length; i++) {
        ...
        if (loaderText)
          loaderText.textContent = `Processando ${i + 1}/${pageState.files.length}: ${file.name}...`;
      ...
      if (processedCount > 0) {
        ...
        showAlert('Erro', 'Nenhum PDF pôde ser processado.');
      }
      if (loaderModal) loaderModal.classList.add('hidden');
    }
  } catch (e: unknown) {
