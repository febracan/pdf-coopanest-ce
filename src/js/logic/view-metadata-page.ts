...
async function displayMetadata() {
  ...
  try {
    ...
    if (info && Object.keys(info).length > 0) {
      ...
      infoSection.ul.innerHTML = `<li><span class="text-gray-500 italic">- Nenhum dado de dicionário de informações encontrado -</span></li>`;
    }
    ...
    if (fieldObjects && Object.keys(fieldObjects).length > 0) {
      for (const fieldName in fieldObjects) {
        ...
        const value = field.fieldValue || '- Não definido -';
    ...
    if (rawXmpString) {
      try {
        ...
        if (xmpSection.ul.children.length === 0) {
          xmpSection.ul.innerHTML = `<li><span class="text-gray-500 italic">- Nenhuma propriedade XMP analisável encontrada -</span></li>`;
        }
      } catch (xmlError) {
        console.error('Failed to parse XMP XML:', xmlError);
        xmpSection.ul.innerHTML = `<li><span class="text-red-500 italic">- Erro ao analisar o XML do XMP. Exibindo dados brutos. -</span></li>`;
    ...
    showAlert(
      'Erro',
      'Não foi possível analisar o PDF por completo. Ele pode estar corrompido ou ter uma estrutura incomum.'
    );
  } finally {
    hideLoader();
  }
...
function copyMetadataAsJson() {
  ...
  navigator.clipboard
    ...
    .then(function () {
      showAlert(
        'Copiado',
        'Metadados copiados para a área de transferência como JSON.'
      );
    })
    .catch(function (err) {
      ...
      showAlert(
        'Erro',
        'Falha ao copiar os metadados para a área de transferência.'
      );
    });
}
