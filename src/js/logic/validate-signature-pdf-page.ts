...
async function handlePdfFile(file: File): Promise<void> {
  if (
    ...
    showAlert('Arquivo inválido', 'Selecione um arquivo PDF.');
    return;
  }
...
async function handleCertFile(file: File): Promise<void> {
  ...
  if (!hasValidExtension) {
    showAlert(
      ...
      'Selecione um arquivo de certificado .pem, .crt, .cer ou .der.'
    );
    return;
  }
  ...
  try {
    ...
    showAlert(
      'Certificado inválido',
      'Falha ao analisar o arquivo de certificado.'
    );
    resetCertState();
  }
}
...
function updateCertDisplay(): void {
  ...
  metaSpan.innerHTML =
    '<i data-lucide="check-circle" class="inline w-3 h-3 mr-1"></i>Certificado confiável carregado';
...
async function validateSignatures(): Promise<void> {
  ...
  showLoader('Analisando assinaturas...');
  ...
  try {
    ...
    showAlert(
      'Erro',
      'Falha ao validar as assinaturas. O arquivo pode estar corrompido.'
    );
  } finally {
    hideLoader();
  }
...
function displayResults(): void {
  ...
  if (state.results.length === 0) {
    resultsContainer.innerHTML = `
            <div class="bg-gray-700 rounded-lg p-6 text-center border border-gray-600">
                ...
                <h3 class="text-lg font-semibold text-white mb-2">Nenhuma assinatura encontrada</h3>
                <p class="text-gray-400">Este PDF não contém nenhuma assinatura digital.</p>
            </div>
        `;
    createIcons({ icons });
    return;
  ...
  let summaryHtml = `
        <p class="text-gray-300">
            ...
            assinatura${state.results.length > 1 ? 's' : ''} encontrada${state.results.length > 1 ? 's' : ''}
            <span class="text-gray-500">•</span>
            <span class="${validCount === state.results.length ? 'text-green-400' : 'text-yellow-400'}">${validCount} válida${validCount > 1 ? 's' : ''}</span>
        </p>
    `;
  ...
  if (state.trustedCert) {
    summaryHtml += `
            <p class="text-xs text-gray-400 mt-1">
                ...
                Verificação de confiança: ${trustVerified}/${state.results.length} assinaturas verificadas com o certificado personalizado
            </p>
        `;
  }
...
function createSignatureCard(
  ...
  let statusText = 'Assinatura válida';

  if (!result.isValid) {
    if (result.cryptoVerificationStatus === 'unsupported') {
      statusColor = 'text-yellow-400';
      statusIcon = 'alert-triangle';
      statusText = 'Não verificada — Algoritmo de assinatura não suportado';
    } else if (
      ...
      statusIcon = 'x-circle';
      statusText =
        'Inválida — Modificada após a assinatura (cobertura parcial)';
    } else {
      statusColor = 'text-red-400';
      statusIcon = 'x-circle';
      statusText =
        result.cryptoVerified === false
          ? 'Inválida — Falha na verificação criptográfica'
          : 'Assinatura inválida';
    }
  } else if (result.usesInsecureDigest) {
    statusColor = 'text-red-400';
    statusIcon = 'x-circle';
    statusText = 'Digest inseguro (MD5 / SHA-1)';
    ...
    statusText = result.isSelfSigned
      ? 'Autoassinada — Identidade do signatário não verificada'
      : 'Assinatura íntegra — Identidade do signatário não verificada';
  }

  const formatDate = (date: Date) => {
    if (!date || date.getTime() === 0) return 'Desconhecido';
  ...
  if (state.trustedCert) {
    if (result.isTrusted) {
      trustBadge =
        '<span class="text-xs bg-green-900 text-green-300 px-2 py-1 rounded ml-2"><i data-lucide="shield-check" class="inline w-3 h-3 mr-1"></i>Confiável</span>';
    } else {
      trustBadge =
        '<span class="text-xs bg-gray-600 text-gray-300 px-2 py-1 rounded ml-2"><i data-lucide="shield-x" class="inline w-3 h-3 mr-1"></i>Fora da cadeia de confiança</span>';
    }
  }
  ...
  card.innerHTML = `
        <div class="flex items-start justify-between mb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-semibold text-white">Assinatura ${index + 1}</h3>
            ...
            <div class="flex items-center">
                ${
                  result.coverageStatus === 'full'
                    ? '<span class="text-xs bg-green-900 text-green-300 px-2 py-1 rounded">Cobertura total</span>'
                    : result.coverageStatus === 'partial'
                      ? '<span class="text-xs bg-yellow-900 text-yellow-300 px-2 py-1 rounded">Cobertura parcial</span>'
                      : ''
                }${trustBadge}
            </div>
        </div>
        ...
        <div class="space-y-3 text-sm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-gray-400">Assinado por</p>
                ...
                <div>
                    <p class="text-gray-400">Emissor</p>
            ...
            ${
              result.signatureDate
                ? `
                <div>
                    <p class="text-gray-400">Assinado em</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-gray-400">Válido de</p>
                    <p class="text-white">${formatDate(result.validFrom)}</p>
                </div>
                <div>
                    <p class="text-gray-400">Válido até</p>
            ...
            <details class="mt-2">
                ...
                <div class="mt-2 p-3 bg-gray-800 rounded text-xs space-y-1">
                    <p><span class="text-gray-400">Número de série:</span> <span class="text-gray-300 font-mono">${escapeHtml(result.serialNumber)}</span></p>
                    <p><span class="text-gray-400">Algoritmo de digest:</span> <span class="text-gray-300">${escapeHtml(result.algorithms.digest)}</span></p>
                    <p><span class="text-gray-400">Algoritmo de assinatura:</span> <span class="text-gray-300">${escapeHtml(result.algorithms.signature)}</span></p>
                    ${result.errorMessage ? `<p class="text-red-400">Erro: ${escapeHtml(result.errorMessage)}</p>` : ''}
