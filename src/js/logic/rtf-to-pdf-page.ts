...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const convertToPdf = async () => {
    try {
      if (state.files.length === 0) {
        showAlert('Nenhum arquivo', 'Selecione ao menos um arquivo RTF.');
        hideLoader();
        return;
      }
      ...
      if (state.files.length === 1) {
        ...
        showLoader('Convertendo vários arquivos RTF para PDF...');
        ...
        showAlert(
          'Conversão concluída',
          `${state.files.length} arquivo(s) RTF convertido(s) para PDF com sucesso.`,
          'success',
          () => resetState()
        );
      }
