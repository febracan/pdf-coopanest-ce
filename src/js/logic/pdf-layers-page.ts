...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateUI = async () => {
    ...
    if (currentFile) {
      ...
      try {
        ...
        metaSpan.textContent = `${formatBytes(currentFile.size)} • Não foi possível carregar a contagem de páginas`;
      }
  ...
  const renderLayers = () => {
    ...
    if (layersArray.length === 0) {
      layersList.innerHTML = `
                <div class="layers-empty">
                    <p>Este PDF não tem camadas (OCG).</p>
                    <p>Adicione uma nova camada para começar!</p>
                </div>
            `;
      return;
    }
        ...
        (layer: LayerData) => `
            <div class="layer-item" data-number="${layer.number}" style="padding-left: ${layer.depth * 24 + 8}px;">
                <label class="layer-toggle">
                    ...
                    <span class="layer-name">${layer.depth > 0 ? '└ ' : ''}${escapeHtml(layer.text || `Camada ${layer.number}`)}</span>
                    ${layer.locked ? '<span class="layer-locked">🔒</span>' : ''}
                </label>
                <div class="layer-actions">
                    ${!layer.locked ? `<button class="layer-add-child" data-xref="${layer.xref}" title="Adicionar subcamada">+</button>` : ''}
                    ${!layer.locked ? `<button class="layer-delete" data-xref="${layer.xref}" title="Excluir camada">✕</button>` : ''}
                </div>
            </div>
        `
      )
      ...
      .forEach((checkbox) => {
        checkbox.addEventListener('change', (e) => {
          ...
          try {
            ...
            target.checked = !isOn;
            showAlert('Erro', 'Falha ao alternar a visibilidade da camada');
          }
        });
      });
    ...
    layersList.querySelectorAll('.layer-delete').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (!layer) {
          showAlert('Erro', 'Camada não encontrada');
          return;
        }
        ...
        try {
          ...
          showAlert('Erro', 'Falha ao excluir a camada');
        }
      });
    });
    ...
    layersList.querySelectorAll('.layer-add-child').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        const childName = await promptForInput(
          'Adicionar subcamada',
          `Digite o nome da subcamada sob "${parentLayer?.text || 'Camada'}":`
        );
        ...
        try {
          ...
          showAlert('Erro', 'Falha ao adicionar a subcamada');
        }
      });
    });
  };
  ...
  const loadLayers = async () => {
    if (!currentFile) {
      showAlert('Nenhum arquivo', 'Selecione um arquivo PDF.');
      return;
    }
    ...
    try {
      ...
      showLoader(`Carregando camadas de ${currentFile.name}...`);
      currentDoc = await (
        pymupdf as { open: (file: File) => Promise<PyMuPDFDocument> }
      ).open(currentFile);

      showLoader('Lendo a configuração de camadas...');
      ...
      showAlert(
        ...
        error instanceof Error
          ? error.message
          : 'Falha ao carregar as camadas do PDF'
      );
  ...
  const setupLayerHandlers = () => {
    ...
    if (addLayerBtn && newLayerInput) {
      addLayerBtn.onclick = () => {
        ...
        if (!name) {
          showAlert('Nome inválido', 'Digite um nome para a camada');
          return;
        }
    ...
    if (saveLayersBtn) {
      saveLayersBtn.onclick = () => {
        try {
          showLoader('Salvando o PDF com as alterações de camadas...');
          ...
          showAlert(
            'Sucesso',
            'PDF com alterações de camadas salvo!',
            'success'
          );
        } catch (err: unknown) {
          hideLoader();
