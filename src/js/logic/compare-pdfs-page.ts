...
function renderChangeList() {
  ...
  if (!comparison || visibleChanges.length === 0) {
    emptyState.textContent =
      comparison?.status === 'match'
        ? 'Nenhuma diferença detectada nesta página.'
        : 'Nenhuma alteração corresponde ao filtro atual.';
  ...
  const typeLabels: Record<string, string> = {
    ...
    modified: 'Modificado',
    moved: 'Movido',
    'style-changed': 'Estilo alterado',
    'page-added': 'Página adicionada',
    'page-removed': 'Página removida',
  };
...
async function buildPagePairs() {
  ...
  showLoader('Construindo modelo de pareamento de páginas...', 0);

  const leftSignatures = await extractDocumentSignatures(
    pageState.pdfDoc1,
    function (pageNumber, totalPages) {
      showLoader(
        `Indexando página ${pageNumber} de ${totalPages} do PDF 1...`,
  ...
  const rightSignatures = await extractDocumentSignatures(
    ...
    function (pageNumber, totalPages) {
      showLoader(
        `Indexando página ${pageNumber} de ${totalPages} do PDF 2...`,
...
async function renderBothPages() {
  ...
  showLoader(
    `Carregando comparação ${pageState.currentPage} de ${pageState.pagePairs.length}...`
  );
...
async function handleFileInput(
  ...
  async function handleFile(file: File) {
    if (!file || file.type !== 'application/pdf') {
      showAlert('Arquivo inválido', 'Selecione um arquivo PDF válido.');
      return;
    }
    ...
    try {
      ...
      if (!result) return;
      showLoader(`Carregando ${result.file.name}...`);
  ...
  if (exportDropdownBtn && exportDropdownMenu) {
    ...
    document.querySelectorAll('.export-menu-item').forEach(function (btn) {
      btn.addEventListener('click', async function () {
        ...
        exportDropdownMenu.classList.add('hidden');
        try {
          showLoader('Preparando exportação do PDF...');
          ...
          showAlert(
            'Erro na exportação',
            'Não foi possível exportar o PDF de comparação.'
          );
        } finally {
          hideLoader();
        }
