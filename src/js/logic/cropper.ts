...
async function displayPageAsImage(num: number) {
  ...
  try {
    ...
    image.onload = () => {
      ...
      enableControls();
      hideLoader();
      showAlert('Pronto', 'Selecione uma área para recortar.');
    };
  } catch (error) {
...
export async function setupCropperTool() {
  ...
  try {
    ...
    showAlert('Erro', 'Falha ao carregar o PDF para recorte.');
  }
  ...
  document.getElementById('crop-button').addEventListener('click', async () => {
    ...
    try {
      ...
      showAlert('Sucesso', 'Recorte concluído! O download foi iniciado.');
    } catch (e) {
      console.error(e);
