...
async function handleFiles(files: FileList) {
  ...
  if (!file || file.type !== 'application/pdf') {
    showAlert('Arquivo inválido', 'Envie um arquivo PDF válido.');
    return;
  }
  try {
    ...
    if (editorFileInfo) {
      editorFileInfo.textContent = `${file.name} (${formatBytes(file.size)}, ${totalPageCount} páginas)`;
    }
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(pageState.file.size)} • ${pageState.pdfDoc.getPageCount()} páginas`;
...
async function applyWatermark() {
  if (!pageState.pdfDoc || !pageState.pdfBytes) {
    showAlert('Erro', 'Envie um arquivo PDF primeiro.');
    return;
  }

  showLoader("Adicionando marca d'água...");
  ...
  try {
    ...
    if (applyToAllPages) {
      ...
      if (config.type === 'text') {
        ...
        if (!imageFile)
          throw new Error(
            "Selecione um arquivo de imagem para a marca d'água."
          );
        ...
        if (imageFile.type === 'image/png') {
          ...
          throw new Error(
            "Imagem não suportada. Use um PNG ou JPG para a marca d'água."
          );
        }
    ...
    if (flattenCheckbox?.checked) {
      ...
      for (let i = 1; i <= totalPages; i++) {
        showLoader(`Achatando página ${i} de ${totalPages}...`);
    ...
    showAlert(
      ...
      e instanceof Error
        ? e.message
        : "Não foi possível adicionar a marca d'água."
    );
  } finally {
    hideLoader();
  }
