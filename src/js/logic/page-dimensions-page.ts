  ...
  let summaryHTML = `
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
      <div class="bg-gray-900 border border-gray-700 rounded-lg p-4">
        <p class="text-sm text-gray-400 mb-1">Total de páginas</p>
        <p class="text-2xl font-bold text-white">${stats.totalPages}</p>
      </div>
      <div class="bg-gray-900 border border-gray-700 rounded-lg p-4">
        <p class="text-sm text-gray-400 mb-1">Tamanhos de página únicos</p>
      ...
      <div class="bg-gray-900 border border-gray-700 rounded-lg p-4">
        ...
        <p class="text-2xl font-bold ${stats.hasMixedSizes ? 'text-yellow-400' : 'text-green-400'}">
          ${stats.hasMixedSizes ? 'Tamanhos variados' : 'Uniforme'}
        </p>
      </div>
    </div>
  `;
    ...
    summaryHTML += `
      <div class="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h4 class="text-yellow-200 font-semibold mb-2">Tamanhos de página variados detectados</h4>
            <p class="text-sm text-gray-300 mb-3">Este documento contém páginas com dimensões diferentes:</p>
            <ul class="space-y-1 text-sm text-gray-300">
              ${stats.uniqueSizes
                .map(
                  (size: UniqueSizeEntry) => `
                <li>• ${size.label}: ${size.count} página${size.count > 1 ? 's' : ''}</li>
              `
                )
                .join('')}
            </ul>
