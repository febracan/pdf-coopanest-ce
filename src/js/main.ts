...
const isMacPlatform = () => navigator.userAgent.toUpperCase().includes('MAC');
...
function getReservedShortcutWarning(
  ...
  return (isMac ? reserved.mac : reserved.windows) || null;
}
...
function showWarningModal(
  ...
  return new Promise((resolve) => {
    ...
    dom.warningModal.addEventListener(
      'click',
      (e) => {
        if (e.target !== dom.warningModal) return;
        if (confirmMode) handleCancel();
        else handleConfirm();
      },
      { once: true }
    );
...
function renderShortcutsList() {
  ...
  const shortcutCategories = getEnabledCategories();
        ...
        if (!isModifier) {
          ...
          if (existingToolId && existingToolId !== toolId) {
            const existingTool = allTools.find(
              (candidate) => getToolId(candidate) === existingToolId
            );
...
function hideBrandingSections() {
  const hideById = (id: string) => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  };

  hideById('hero-section');
  ...
  hideById('features-section');
  hideById('security-compliance-section');
  hideById('faq-accordion');
  hideById('testimonials-section');
  hideById('support-section');
...
function renderToolGrid(toolGrid: HTMLElement) {
  ...
  getEnabledCategories().forEach((category) => {
  ...
  searchBar.addEventListener('input', () => {
    ...
    categoryGroups.forEach((group) => {
      Array.from(group.querySelectorAll('.tool-card')).forEach((card) => {
        const toolName = (
...
function setupFullWidthToggle() {
  ...
  const applyFullWidthMode = (enabled: boolean) => {
    if (toolInterface) {
      toolInterface.classList.toggle('max-w-4xl', !enabled);
    }

    document
      .querySelectorAll('#tool-uploader, #signature-editor')
      .forEach((uploader) => {
        if (enabled) {
...
function setupSettingsTabs() {
  ...
  if (!shortcutsTabBtn || !preferencesTabBtn) return;
...
function loadGithubStars() {
  ...
  if (!githubStarsElements.some((el) => el) || __SIMPLE_MODE__) return;
...
function renderDisabledToolPage() {
...
const init = async () => {
  ...
  // iPad excluded so keyboard users keep shortcuts.
  ...
  } else if (keyboardShortcutBtn) {
    keyboardShortcutBtn.textContent = isMacPlatform() ? '⌘ + K' : 'Ctrl + K';
  }

  if (dom.toolGrid) {
    renderToolGrid(dom.toolGrid);
  }
