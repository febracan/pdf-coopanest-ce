...
export const switchView = (view: string) => {
  if (view === 'grid') {
    ...
    dom.dividers.forEach((divider) => divider.classList.remove('hidden'));
    dom.hideSections.forEach((section) => section.classList.remove('hidden'));
    ...
    dom.dividers.forEach((divider) => divider.classList.add('hidden'));
    dom.hideSections.forEach((section) => section.classList.add('hidden'));
  }
};
  ...
  const createWrapper = (canvas: HTMLCanvasElement, pageNumber: number) => {
    ...
    if (toolId === 'organize') {
      ...
      const updateRotation = (newRotation: number) => {
        ...
        wrapper.dataset.rotation = newRotation.toString();
        imgEl.style.transform = `rotate(${newRotation}deg)`;
        updateRotationState(pageNumber - 1, newRotation);
      };
