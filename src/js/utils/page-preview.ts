...
function getOrCreateModal(): HTMLElement {
  ...
  modal.innerHTML = `
    <button id="preview-close" class="absolute top-4 right-4 text-white/70 hover:text-white z-10 transition-colors" title="Fechar (Esc)">
    ...
    <button id="preview-prev" class="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-white transition-colors p-2" title="Página anterior">
    ...
    <button id="preview-next" class="absolute right-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-white transition-colors p-2" title="Próxima página">
    ...
    <div id="preview-canvas-container" class="flex items-center justify-center max-w-[90vw] max-h-[85vh]">
      <div id="preview-loading" class="text-white/60 text-sm">Carregando...</div>
    </div>
...
async function renderPreviewPage(pageNumber: number): Promise<void> {
  ...
  container.innerHTML =
    '<div class="text-white/60 text-sm">Carregando...</div>';

  pageInfo.textContent = `Página ${pageNumber} de ${state.totalPages}`;
  ...
  try {
    ...
    container.innerHTML =
      '<div class="text-red-400 text-sm">Falha ao renderizar a página</div>';
  }
}
