    ...
    <div class="bg-gray-800 rounded-xl border border-gray-700 shadow-2xl max-w-md w-full overflow-hidden">
      <div class="p-6">
        <div class="flex items-start gap-4 mb-4">
          ...
          <div class="flex-1">
            <h3 id="password-modal-title" class="text-xl font-bold text-white mb-1">Senha obrigatória</h3>
      ...
      <div class="p-6 overflow-y-auto flex-1 min-h-0">
        <div class="flex items-start gap-4 mb-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-gray-400 text-sm">Digite as senhas de cada arquivo criptografado</p>
          </div>
        </div>
        <div class="flex items-center gap-2 mt-3 mb-3">
          ...
          <label for="batch-modal-same-pw" class="text-sm text-gray-300 cursor-pointer select-none">Usar a mesma senha para todos os arquivos</label>
        </div>
        <div id="batch-modal-shared" class="mb-3">
          <div class="relative">
            <input type="password" id="batch-modal-shared-input"
              class="w-full bg-gray-700 border border-gray-600 text-gray-200 rounded-lg px-4 py-2.5 pr-10 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              placeholder="Senha para todos os arquivos" autocomplete="off" />
...
export async function promptAndDecryptFile(file: File): Promise<File | null> {
  ...
  if (titleEl) titleEl.textContent = 'Senha obrigatória';
  ...
  submitBtn.dataset.originalText = 'Desbloquear';
  ...
  const modalPromise = new Promise<File | null>((resolve) => {
    ...
    async function onSubmit() {
      ...
      if (progressEl) {
        progressEl.textContent = 'Validando...';
      ...
      if (progressEl) progressEl.textContent = 'Descriptografando...';
      ...
      try {
        ...
        if (errorEl) {
          errorEl.textContent =
            'Falha ao descriptografar. Tente usar a ferramenta Descriptografar.';
          errorEl.classList.remove('hidden');
        }
      }
    }
...
export async function promptAndDecryptBatch(
  ...
  if (titleEl)
    titleEl.textContent = `${fileNames.length} arquivos precisam de senha`;
      ...
      (name, i) =>
        `<div class="flex items-center gap-2 p-2 bg-gray-700/50 rounded-lg transition-all" data-file-idx="${i}">
          ...
          <div class="flex items-center gap-1.5 flex-shrink-0">
            <input type="password" data-pw-idx="${i}" placeholder="Senha"
            ...
            <button type="button" data-skip-idx="${i}" title="Pular este arquivo"
  ...
  submitBtn.disabled = false;
  submitBtn.textContent = 'Desbloquear todos';
  submitBtn.dataset.originalText = 'Desbloquear todos';
  cancelBtn.disabled = false;
  ...
  const batchPromise = new Promise<Map<number, File>>((resolve) => {
    ...
    function updateButtons(autoClose = false) {
      ...
      if (hasSucceeded) {
        submitBtn.textContent = `Desbloquear os restantes (${remaining})`;
    ...
    function onSkipFile(e: Event) {
      ...
      if (skippedSet.has(idx)) {
        ...
        const pwInput =
          row.querySelector<HTMLInputElement>('input[data-pw-idx]');
        if (pwInput) pwInput.disabled = true;
        btn.title = 'Incluir este arquivo';
      }
      updateButtons(true);
    }
    ...
    async function onSubmit() {
      ...
      for (let i = 0; i < fileNames.length; i++) {
        ...
        if (!password) {
          if (errorEl) {
            const msg = useSame
              ? 'Digite uma senha'
              : `Digite uma senha para ${fileNames[i]} ou pule-o`;
      ...
      for (let i = 0; i < toProcess.length; i++) {
        ...
        if (progressEl) {
          progressEl.textContent = `Validando ${i + 1} de ${toProcess.length}: ${files[realIdx].name}`;
          progressEl.classList.remove('hidden');
        }
        ...
        if (progressEl) {
          progressEl.textContent = `Descriptografando ${i + 1} de ${toProcess.length}: ${files[realIdx].name}`;
        }
      ...
      if (failedNames.length > 0) {
        if (errorEl) {
          errorEl.textContent = `Senha incorreta para: ${failedNames.join(', ')}`;
          errorEl.classList.remove('hidden');
        }
        ...
        updateButtons();
        submitBtn.textContent = 'Repetir com falha';
        return;
      }
