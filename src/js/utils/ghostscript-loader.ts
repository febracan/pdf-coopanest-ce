...
export async function convertToPdfA(
  ...
  if (!isWasmAvailable('ghostscript')) {
    throw new Error(
      'O Ghostscript não está configurado. Configure-o nas Configurações WASM.'
    );
  }

  onProgress?.('Carregando Ghostscript...');
  ...
  onProgress?.(`Convertendo para ${level}...`);
...
`;
    ...
    throw new Error(
      'Falha na conversão: não foi possível criar a definição PDF/A',
      {
        cause: e,
      }
    );
  ...
  try {
    ...
    throw new Error('O Ghostscript não gerou o arquivo de saída', { cause: e });
  }
  ...
  if (level !== 'PDF/A-1b') {
    onProgress?.('Pós-processando para conformidade de transparência...');
    console.log(
...
export async function convertFontsToOutlines(
  ...
  onProgress?.('Convertendo fontes em contornos...');
