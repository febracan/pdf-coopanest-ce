...
class WasmProviderManager {
  ...
  async validateUrl(
    ...
    if (!testUrl) {
      return { valid: false, error: 'Nenhuma URL configurada' };
    }
    ...
    try {
      ...
      return {
        valid: false,
        error:
          'Formato de URL inválido. Digite uma URL válida (ex.: https://example.com/wasm/)',
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        return {
          valid: false,
          error: `Não foi possível encontrar ${testFile} na URL especificada (HTTP ${response.status}). Verifique se o arquivo existe.`,
        };
      }
      ...
      if (reader) {
        try {
          ...
          return {
            valid: false,
            error: `O arquivo existe, mas não pôde ser lido. Verifique a configuração de CORS.`,
          };
        }
      }
      ...
      if (
        ...
        return {
          valid: false,
          error: `A URL retornou um tipo de conteúdo inesperado: ${contentType}. Esperava-se um arquivo JavaScript.`,
        };
      }
      ...
      if (
        ...
        return {
          valid: false,
          error:
            'Erro de rede: não foi possível conectar à URL. Verifique se a URL está correta e se o servidor permite requisições CORS.',
        };
      }
  ...
  getPackageDisplayName(packageName: WasmPackage): string {
    const names: Record<WasmPackage, string> = {
      pymupdf: 'PyMuPDF (Processamento de documentos)',
      ghostscript: 'Ghostscript (Conversão para PDF/A)',
      cpdf: 'CoherentPDF (Marcadores e metadados)',
    };
  ...
  getPackageFeatures(packageName: WasmPackage): string[] {
    const features: Record<WasmPackage, string[]> = {
      pymupdf: [
        ...
        'PDF para Markdown',
        'PDF para SVG',
        'PDF para Imagens (Alta qualidade)',
        'PDF para DOCX',
        'PDF para Excel/CSV',
        'Extrair imagens',
        'Extrair tabelas',
        'EPUB/MOBI/FB2/XPS/CBZ para PDF',
        'Compressão de imagens',
        'Corrigir inclinação do PDF',
        'Camadas do PDF',
      ],
      ghostscript: ['Conversão para PDF/A', 'Fonte para contorno'],
      cpdf: [
        'Mesclar PDF',
        'Mesclagem alternada',
        ...
        'PDF para JSON',
        'JSON para PDF',
        'Adicionar/Editar/Extrair anexos',
        'Editar marcadores',
        'Metadados do PDF',
      ],
    };
  ...
  modal.innerHTML = `
    <div class="p-6">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="text-lg font-semibold text-white">Recurso avançado necessário</h3>
          <p class="text-sm text-gray-400">É necessário um módulo de processamento externo</p>
        </div>
      </div>

      <p class="text-gray-300 mb-4">
        Este recurso requer que o <strong class="text-white">${displayName}</strong> esteja configurado.
      </p>

      <div class="bg-gray-700/50 rounded-lg p-4 mb-4">
        <p class="text-sm text-gray-400 mb-2">Recursos habilitados por este módulo:</p>
        <ul class="text-sm text-gray-300 space-y-1">
          ${features
            ...
            .join('')}
          ${features.length > 4 ? `<li class="text-gray-500">+ ${features.length - 4} mais...</li>` : ''}
        </ul>
      </div>

      <p class="text-xs text-gray-500 mb-4">
        Este módulo é licenciado sob AGPL-3.0. Ao configurá-lo, você concorda com os termos da licença.
      </p>
    </div>
