  ...
  async initialize(onProgress?: ProgressCallback): Promise<void> {
    ...
    try {
      progressCallback?.({
        phase: 'loading',
        percent: 0,
        message: 'Carregando mecanismo de conversão...',
      });
      ...
      this.converter = new WorkerBrowserConverter({
        ...
        onProgress: (info: {
          ...
          if (progressCallback && !this.initialized) {
            const simplifiedMessage = `Carregando mecanismo de conversão (${Math.round(info.percent)}%)...`;
            progressCallback({
      ...
      progressCallback?.({
        phase: 'ready',
        percent: 100,
        message: 'Mecanismo de conversão pronto!',
      });
